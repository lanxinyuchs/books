...
%%% @version /main/R4A/R5A/R7A/R8A/R10A/R11A/2
...
-vsn('/main/R4A/R5A/R7A/R8A/R10A/R11A/2').
-date('2017-10-12').
-author('ecotjos').
...
%%% R5A/12     2016-04-12 etxtory     sysDbServer force autobackup
%%% -----   ---------  -------- ------------------------
%%% R8A/1      2016-11-03 emariad     SP341: Encrypt DB for cloud
%%% R8A/2      2016-12-06 erarafo     Fix compiler warnings
%%% R8A/3      2017-01-10 emariad     Removed outcommented encryption, for cloud
%%% -----   ---------  -------- ------------------------
%%% R10A/1  2017-05-09 etxtory  Added get_autobackup_path for AIC
%%% R10A/2  2017-05-22 etxjotj  Don't assume monotonic time starts at 0
%%% R10A/3  2017-05-29 etxjotj  Moved encryption to move_file
%%%                             Added checksum logging
%%%         2017-05-15 etxarnu  Added call to vnfcI:start_heartbeat if vrcs
%%% R10A/4  2017-07-12 edamkon  Implemented fix for HW12530
%%% ----------------------------------------------------------
%%% R11A/1  2017-09-06 edamkon  Increase timeout to 40s in HW12530 fix
%%% R11A/2  2017-10-09 ecotjos  Increase timeout to 60s in HW34193 fix
%%% ----------------------------------------------------------
%%% 
%%% #2.    EXPORT LISTS
%%% ----------------------------------------------------------
...
%% Called at initial installation by sysDbServer.erl
...
get_autobackup_path() ->
    filename:join(sysEnv:home_dir(), "autobackup.gz").

%% Is called before swmDbMonitor process is started.
%% No gen_server:call/cast can be used.
initial_auto_backup() ->
    make_auto_backup(_RunsInProcess = false).
...
-record(state, {timerRef,     %% The current db timer or 'undefined'
		...
		qEndTime= erlang:monotonic_time(),   %% The next quarantine 
		%% period end time as measured in monotonoic time
...
handle_cast(quarantine_reset, State) ->
    ...
    {noreply, State#state{qPeriod = ?Q_PERIOD_DEFAULT,
			  qTimer = NewQTimer,
			  qReset = true,
			  qEndTime = erlang:monotonic_time(),
...
do_extract_imm_table({delete, _, _What, _, _}) ->
    {imm_objects, delete}.
...
make_auto_backup() ->
    make_auto_backup(true).

make_auto_backup(RunsInProcess) ->
    case RunsInProcess of
	true ->
	    flush_events();
	false ->
	    ok
...
%%         {ok, EncryptedData} = certI:encrypt_data(Bin),
%%         file:write_file(Path, term_to_binary({encrypted, EncryptedData})),
%%         ok
%%     catch Type:Reason ->
%% 	    warning([{?MODULE,{"Encryption of database failed ",[{Type, Reason}]}}])
%%     end.


move_file(TmpDir, TmpPath, Path) ->
    {0,_} = swmOs:cmdres(["md5sum ",TmpPath]),
    {0,_} = swmOs:cmdres(["cd ", TmpDir, " ; gzip ", TmpPath]),
    {0,_} = swmOs:cmdres(["file ",TmpPath,".gz ; md5sum ",TmpPath,".gz"]),

    case certI:is_encryption_enabled() of
	true -> %% Encrypt DB
	    {ok, Bin} = file:read_file(TmpPath++".gz"),
	    {ok, EncryptedData} = certI:encrypt_data(Bin),
	    OutBin = term_to_binary({encrypted, EncryptedData}),
	    ok = file:write_file(Path++".new", OutBin),
	    ok = file:delete(TmpPath++".gz"),
	    {0,_} = swmOs:cmdres(["md5sum ", Path, ".new ; sync"]),
	    ok;
	false -> %% Move DB to permanent storage
	    {0,_} = swmOs:cmdres(["mv -f ",TmpPath,".gz ",Path,".new ; sync"])
    end,
