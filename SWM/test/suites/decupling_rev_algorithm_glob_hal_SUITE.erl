%%% #0.    BASIC INFORMATION
%%% ----------------------------------------------------------
%%% %CCaseFile:	decupling_rev_algorithm_glob_hal_SUITE.erl %
%%% @author etxivri
%%% @copyright Ericsson AB 2016
%%% @version /main/R5A/5
%%%
%%% @doc == HW-SW-DEC, verify HW revision selection algorithm. When HAL and Global exist. ==
%%%
%%%
%%% @end

-module(decupling_rev_algorithm_glob_hal_SUITE).
-author('etxivri').
-vsn('/main/R5A/5').
-date('2016-04-14').
...
-export([suite/0,
	 ...
	 test_3_1_1/1,
	 %% no match
	 test_3_2_1/1, % ?? Se comment in TC
	 test_3_2_2/1, % ?? Se comment in TC
	 ...
	 %% Verification Rev
	 test_10_5_1/1,
	 test_10_5_2/1 %% Range is allowed, Not decided if it is ok or not!
	]).
...
%%%%
%% HAL SWP xml.
-define(HAL_XML_1, "HalUP_2_rev_check-hal.xml").
-define(HAL_XML_MAX_REV_RANGE, "HalUP_max_rev_range-hal.xml").
-define(HAL_XML_NOK_1, "HalUP_nok_1-hal.xml").
-define(HAL_XML_NOK_2, "HalUP_nok_2-hal.xml").
-define(HAL_XML_NOK_3, "HalUP_nok_3-hal.xml").
-define(HAL_XML_NOK_4, "HalUP_nok_4-hal.xml").
%%%%
	...
	[{{"TEST-1A","CXP111111_1A","R1A"},
	      {global,"TEST_1A_CXP111111.cxp"}},
            {{"TEST-1B","CXP111111_1B","R1B"},
             {hal,"TEST_1B_CXP111111.cxp"}}]
       ).
-define(RESULT_2, 
	[{{"TEST-2A","CXP222222_2A","R2A"},
	      {hal,"TEST_2A_CXP222222.cxp"}},
...
%% Index Match.
%% KDU match both in HAL and Global.
%% Rev match an explicit R-state in XML.
%% - CXPs from HAL and Global - boardType is used. 
...
test_2_2_1(Config) ->
    ...
    ct:pal("A. HW Spcial rev match special rev, explicit R-state."),
    CxpList = run_mfa_to_get_cxps(Config,
    				  ?REV_MATCH_EXACT_3),
    ct:pal("Reply: ~p ", [CxpList]),
    decupling_lib:match_cxplist(?RESULT_7, CxpList),
    
    ct:pal("B. HW ordinary rev match special rev, explicit R-state."),
    CxpListB = run_mfa_to_get_cxps(Config,
    				  ?REV_MATCH_EXACT_3_B),
    ct:pal("Reply B: ~p ", [CxpListB]),
    decupling_lib:match_cxplist(?RESULT_7, CxpListB),
    
    ct:pal("C. HW special rev match ordinary rev, explicit R-state."),
    CxpListC = run_mfa_to_get_cxps(Config,
    				  ?REV_MATCH_EXACT_3_C),
    ct:pal("Reply C: ~p ", [CxpListC]),
    decupling_lib:match_cxplist(?RESULT_1, CxpListC),
    
    ct:pal("D. HW special rev match ordinary rev, explicit R-state."),
...
test_2_2_2(Config) ->
    ...
    ct:pal("B. HW ordinary rev match special rev, open range."),
    CxpListB = run_mfa_to_get_cxps(Config,
...
test_2_2_3(Config) ->
    ...
    ct:pal("B. HW ordinary rev match special rev, closed range"),
    CxpListB = run_mfa_to_get_cxps(Config,
    				  ?REV_MATCH_CLOSED_RANGE_3_B),
    ct:pal("Reply B: ~p ", [CxpListB]),
    decupling_lib:match_cxplist(?RESULT_9, CxpListB),
    
    ct:pal("C. HW special rev match  ordinary rev, closed range"),
    CxpListC = run_mfa_to_get_cxps(Config,
    				  ?REV_MATCH_CLOSED_RANGE_3_C),
    ct:pal("Reply C: ~p ", [CxpListC]),
    decupling_lib:match_cxplist(?RESULT_3, CxpListC),

    ct:pal("D. HW special rev match  ordinary rev, closed range"),
    CxpListD = run_mfa_to_get_cxps(Config,
...
test_2_2_12(Config) ->
    scp_xml(Config, ?XML_PATH_1, ?GLOBAL_XML_1),
    scp_xml(Config, ?XML_PATH_1, ?HAL_XML_NOK_1),
...
%% Index Match.
%% KDU match both in HAL and Global.
%% Preliminary Rev not match an open range in XML.
%% - Result: ?? This will match an range and it OK until it is sorted out
%%              how it shall work.
...
%% Index Match.
%% KDU match.
%% Rev using max allowed alpabetic letters match an explicit revision
%% in HAL and Global XML.
%% - CXPs from HAL and Global - boardType is used. 
%% - Expected CXPs = ?RESULT_1
...
test_10_1_1(Config) ->
    ...
    scp_xml(Config, ?XML_PATH_1, ?HAL_XML_MAX_REV_RANGE),
...
%% Index Match.
%% KDU match.
%% Rev using max allowed digits match an explicit revision 
%% in HAL and Global XML.
%% - CXPs from HAL and Global - boardType is used. 
%% - Expected CXPs = ?RESULT_2
...
%% Index Match.
%% KDU match.
%% Rev exceed max allowed alpabetic letters in HAL XML.
%% - Reslt: error
...
test_10_1_5(Config) ->
    scp_xml(Config, ?XML_PATH_1, ?GLOBAL_XML_MAX_REV_RANGE),
    scp_xml(Config, ?XML_PATH_1, ?HAL_XML_NOK_2),
...
%% Index Match.
%% KDU match.
%% Rev using max allowed alpabetic letters match an open ended revision 
%% HAL and Globalin XML.
%% - CXPs from HAL and Global - boardType is used. 
%% - Expected CXPs = ?RESULT_1
...
%% Index Match.
%% KDU match.
%% Rev using max allowed digits match an open ended revision
%% HAL and Global in XML.
%% - CXPs from HAL and Global - boardType is used. 
%% - Expected CXPs = ?RESULT_2
...
%% Index Match.
%% KDU match.
%% Rev using max allowed alpabetic letters match an closed revision
%% in HAL and Global XML.
%% - CXPs from HAL and Global - boardType is used. 
%% - Expected CXPs = ?RESULT_1
...
%% Index Match.
%% KDU match.
%% Rev using max allowed digits match an closed revision
%% in HAL and Global XML.
...
%% KDU match.
%% Not allowed letter combination in HAL XML.
%% - Reslt: error
...
test_10_4_2(Config) ->
    scp_xml(Config, ?XML_PATH_1, ?GLOBAL_XML_1),
    scp_xml(Config, ?XML_PATH_1, ?HAL_XML_NOK_3),
...
%% Only Global UP exist. 
%% KDU match.
%% Verification Rev NOT allowed as a range in XML.
%% - error due to no rev match
...
test_10_5_2(Config) ->
    scp_xml(Config, ?XML_PATH_1, ?GLOBAL_XML_1),
    scp_xml(Config, ?XML_PATH_1, ?HAL_XML_NOK_4),
    ...
    CxpList2 = run_mfa_to_get_cxps(Config,
				   HW_2),
    %% ?UC_ERR_2(?ERROR_CAUSE_3) = CxpList2,
    decupling_lib:match_cxplist(?RESULT_12, CxpList2),

    ct:pal(" # Test 3"),
    HW_3 = {"KDU111111/2","R30A10"},
    CxpList3 = run_mfa_to_get_cxps(Config,
				   HW_3),
    %% ?UC_ERR_2(?ERROR_CAUSE_3) = CxpList3.
    decupling_lib:match_cxplist(?RESULT_12, CxpList3).
