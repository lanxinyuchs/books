...
do_make_backup(Scheduler) ->
    ...
    case swmBackup:validate_name(list_to_binary(Name)) of
	ok ->
	    ok;
	duplicate_name ->
    ...
    DN = swmBackup:create_backup_common(
	...
	?BrmAutoExport_ENABLED ->
	    ...
	    gen_server:call(swmBackup,
			    {export, BuKey, Cleartext, Uri, ActionId},
			    3600000),
	...
	{cleaned, Indices} ->
	    [begin 
		 ...
		 Msg = "Backup "++BackupName++" removed automatically "
		     "due to housekeeping.",
		 swmBackup:update_progress(schedule, [{additionalInfo, Msg}])
