...
%%% @version /main/R2A/R3A/R4A/R5A/11
...
do_action(<<"deactivate">>, _DnRev, _, _) ->
    gen_server:cast(swmFailsafe, {deactivate, "BrmFailsafeBackup"}),
    ?INT32(?actionStarted).
...
handle_cast({deactivate, Dn}, State) ->
    case get_state() of
    ?UsageState_BUSY ->
        case swmLib:lock_action_capable(?SWM_FAILSAFE_ACTION_CAPABLE_ID, 
        ...
        ok ->
            ...
            Id = swmLib:get_new_action_id(failsafe),
            swmBackup:set_default_progress_report(
              failsafe, "DEACTIVATE", Id), % HU36796
            BackupName = update_db_deactivate(),
        %% Tmporarily backed out because awaiting TN test update
        ...
        %% swmDbMonitor:force_auto_backup(),
            ...
            swmBackup:update_progress(failsafe, 
        ...
        {nok, _}->
           Msg = "Failsafe configuration function failed to deactivate. " 
                    ++ swmLib:get_action_capable_info(),
           CompleteTime = comsaI:iso_time(os:timestamp(), extended),
           swmBackup:update_progress(failsafe, 
...
create_backup(Id, BackupName) ->
    swmBackup:set_default_progress_report(failsafe, "ACTIVATE", Id), % HU36796
    ManagerKey = {"1","1","1","1"},
