...
getMoAttribute([<<"timeRemainingBeforeFallback">>|_DnRev], _) ->
	...
	undefined -> 
	    comsaEcimModelAdaptor:type(
	      int16,?swM_timeRemainingBeforeFallback_default);
...
update_sw_version(UpFile, ActiveState) ->
	...
	Key = swmInventory:make_mom_key(ProdId, Version),
	Key
    catch T:E ->
	    Dir = filename:basename(filename:dirname(UpFile)),
	    logI:write_log("SwmInternal", "SwInventory", error, 
			   "Corrupt UP file in "++Dir),
	    sysInitI:error_report([{xmerl_scan, file, [UpFile]},{T,E}]),
	    undefined
    end.
...
init_upgrade_package(ActiveKey, UpFile) ->
	...
	{ConfigurationE, []} ->
		...
		case mnesia:read({upgradePackage, UpgradePackageId}) of
		    [O] -> 
			{State, Created} = 
			    do_determine_package_state([O], UpKey, ActiveKey,
						       IsUpgrade, IsComplete),
			O#upgradePackage{state=State,
					 created=Created};
		    [] -> 
			{State, Created} = 
	    ...
	    sysInitI:error_report([{xmerl_scan, file, [UpFile]},{T,E}]),
	    Cmd = "rm -rf "++filename:dirname(UpFile),
...
do_determine_package_state([], _, _, _, IsComplete) ->
    %% No previous entry exists. 
    %% If the up is not the active, then it must be a new UP. 

    State = if IsComplete -> ?UpgradePackageState_PREPARE_COMPLETED;
	       true -> ?UpgradePackageState_INITIALIZED
	    end,
...
do_determine_package_state([OldObj], UpKey, ActiveKey, IsUpgrade, IsComplete) ->
	...
	case State of
		...
		if IsComplete -> ?UpgradePackageState_PREPARE_COMPLETED;
		   true -> ?UpgradePackageState_INITIALIZED
		end;
