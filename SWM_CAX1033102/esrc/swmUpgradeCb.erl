...
verify_precondition() ->
    ...
    os:cmd("cup --upgrade verifyPreconditions"),
    ...
    case FreeDisk of
    	FreeDisk when FreeDisk > NeededSize ->
    	    ok;
    	FreeDisk ->
    	    {ok, "There is disk space shortage. Remove backups or old upgrade packages to make room. "++
		 integer_to_list(NeededSize div 1024)++" k needed. "++
		 integer_to_list(FreeDisk div 1024)++" k available"}
    end.

verify_upgrade() ->
    os:cmd("cup --upgrade verifyUpgrade"),
    ok.

activate_start() ->
    os:cmd("cup --upgrade activateStart"),
    ok.

preload() ->
    os:cmd("cup --upgrade preload"),
    ok.

activate() ->
    os:cmd("cup --upgrade activate"),
    ok.

confirm() ->
    os:cmd("cup --upgrade confirm"),
    ok.
