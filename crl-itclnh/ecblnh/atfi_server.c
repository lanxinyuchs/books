...
#define ECB_MBOX_WATERMARK_MINIMUM 100
/* Number of messages to receive before checking mailbox watermark */
#define ECB_MBOX_RCV_DIAG_DELAY 100
/* Number of messages to receive before reporting how many RHAI and
 * application messages there was */
#define ECB_MBOX_RCV_DIAG_LOG_DELAY 100000
...
union itc_msg {
	...
	struct ulh_lnhmsg_enable_diag			endableDiag;
...
struct atf_data atfi = { .mpa4ci_mbox = ITC_NO_ID };
...
do_connect(struct ecb_link_config *cfg, struct atf_conn *conn)
{
    ...
    sprintf(msg->cmreq.cm_instance, "%s_%02X", cfg->name, (char)cfg->address);
...
do_disconnect(struct atf_conn *conn)
{
	...
	ret = ulh_trans_destroy_conn(conn->tran_id);
...
int main(int argc, char *argv[])
{
...
#ifdef FLOW_DIAG
			msg = itc_alloc(sizeof(msg->endableDiag), ULH_LNHMSG_ENABLE_DIAG);
			msg->endableDiag.mailbox_watermark_minimum = ECB_MBOX_WATERMARK_MINIMUM;
			msg->endableDiag.mailbox_watermark_delay = ECB_MBOX_RCV_DIAG_DELAY;
			msg->endableDiag.mailbox_msg_type_delay = ECB_MBOX_RCV_DIAG_LOG_DELAY;
			itc_send(&msg, atfi.server_mbox, ITC_MY_MBOX);
#endif
			break;
		}
	...
	(void)pthread_setname_np(pthread_self(), "atfi_server");
