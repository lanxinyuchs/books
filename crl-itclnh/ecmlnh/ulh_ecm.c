	...
	if(ECM_MAIN_GET_VER(hdr) != ECM_VER_3) {
        ...
        if(ECM_MAIN_GET_NEXT(hdr) != ECM_CONN &&
           ECM_MAIN_GET_CONNID(hdr) != co->cid) {
                if (!co->shutdown) {
                        ULH_TRACE_ERROR("Packet received from incorrect connection %d should be %d", ECM_MAIN_GET_CONNID(hdr), co->cid);
                }
                else {
                        ULH_TRACE_INFO("Packet received from incorrect connection %d should be %d", ECM_MAIN_GET_CONNID(hdr), co->cid);
                }
                return ECM_LAST;
        }
			       ...
			       struct ulh_timerqueue *tqueue)
{
	...
	ulh_tbuff_pool_init(&co->tbpool, TB_BUFFER_SIZE);
