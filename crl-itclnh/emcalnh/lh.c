...
#define PTR_MASK                                                (~(uintptr_t)0x0000003F)
...
static void notify_owner(struct mhp_link *link, uint32_t state)
{
	...
	} else if (link->link_mbox != ITC_NO_ID) {
		...
		struct rlnh_msg_publish *msg)
{
	...
	if (laddr > MAX_REP_ADDR)
	{
		MHP_ERROR("[LNH]: Bad publish adress laddr: %d  src: %d)", laddr);
		return;
	}
		...
		struct rlnh_msg_unpublish *msg)
{
	...
	if (laddr > MAX_REP_ADDR)
	{
		MHP_ERROR("[LNH]: Bad unpublish adress laddr: %d  src: %d)", laddr);
		return;
	}
												...
												union itc_msg* msg)
{
	...
	if (lep) {
		...
		MHP_INFO("[LNH]: msgno 0x%x from 0x%x to unknown dst_addr %d",
...
static void handle_create(union itc_msg *cfg)
{
	...
	if (ret) {
		MHP_INFO("[LNH]: handle_create failed on collision_detect, ret: %d",
				ret);
		goto done;
	}
	...
	if (ret) {
		MHP_INFO("[LNH]: handle_create failed on alloc_link, ret: %d", ret);
		goto done;
	}
	...
	if (mhp == NULL) {
		MHP_INFO("[LNH]: handle_create failed on malloc");
		goto done;
	}
	...
	if (ret) {
		MHP_INFO("[LNH]: handle_create failed on ecom_init, ret: %d", ret);
		goto done;
	}
...
static void handle_remotemsg(union itc_msg *msg, itc_mbox_id_t to)
{
	...
	syslog(LOG_WARNING, "[LNH]: remote msg LEP (id = %p): %p REP: %p",
