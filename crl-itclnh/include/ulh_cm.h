...
/* for sending commands/messages to CM,
* that will be proccessed on lnh thread
*/
#define ULH_CMMSG_GENERAL	(0x0190042d)
struct ulh_cmmsg_general {
	uint32_t msg_no;
	uint32_t link_id;
	uint32_t cm_msg_no;
};
...
struct ulh_cm_config {
	...
	itc_mbox_id_t mbox; /* UC mbox */
        uint32_t max_itc_msg_size; /* maximal size of message that can be sent to user  */
};
