...
static itc_mbox_id_t owner_mboxes[MAX_HANDLES];

static inline void set_handle_owner(uint32_t handle, itc_mbox_id_t mbox)
{
	if (handle >= MAX_HANDLES) {
		printf("Handle too large. %d > %d\n", handle, MAX_HANDLES);
		exit(-1);
	}

	owner_mboxes[handle] = mbox;
}

static inline itc_mbox_id_t get_handle_owner(uint32_t handle)
{
	if (handle >= MAX_HANDLES) {
		printf("Handle too large. %d > %d\n", handle, MAX_HANDLES);
		exit(-1);
...
static inline uint32_t htonl(uint32_t in)
{
	return htobe32(in);
}
		...
		struct lhsh_rcmd *rcmd)
{
   ...
   sig->create_child_req.terminal  = htonl(str_add(sig->create_child_req.s, remain_len - soff, "", &soff));
	...
	/*
	 * Set ownership of the three handles that we pass to the child.
	 */
	itc_mbox_id_t mbox = itc_sender(sig);
	set_handle_owner(1, mbox);
	set_handle_owner(2, mbox);
	set_handle_owner(3, mbox);
...
static int rcmd_write_req(union itc_msg *sig)
{
	union itc_msg *rsig;
...
static int rcmd_read_req(union itc_msg *sig)
{
	int ret;
...
static int rcmd_if_req(union itc_msg *sig)
{
	union itc_msg *rsig;
...
static int rcmd_dup_req(union itc_msg *sig)
{
        union itc_msg *rsig;
	FmHandle oldHandle = ntohl(sig->fm_dup_handle.oldHandle);
	FmHandle newHandle = oldHandle + 0x10;
	itc_mbox_id_t oldHandleOwner = get_handle_owner(oldHandle);
	itc_mbox_id_t newHandleOwner = itc_sender(sig);

	set_handle_owner(newHandle, newHandleOwner);

	rsig = itc_alloc(sizeof(struct FmDupHandle),FM_DUP_HANDLE_REPLY);

	rsig->fm_dup_handle.status = 0;
	rsig->fm_dup_handle.oldHandle = htonl(oldHandle);
	rsig->fm_dup_handle.newHandle = htonl(newHandle);

	itc_send(&rsig, oldHandleOwner, ITC_MY_MBOX);
	return 0;
}

static int rcmd_owner_req(union itc_msg *sig)
{
        union itc_msg *rsig;
	FmHandle handle = ntohl(sig->fm_setowner_req.handle);
	itc_mbox_id_t old_owner = get_handle_owner(handle);
	itc_mbox_id_t new_owner = itc_sender(sig);

	set_handle_owner(handle, new_owner);

	rsig = itc_alloc(sizeof(struct FmSetOwnerProc),
			FM_SET_OWNER_PROC_REPLY);

	rsig->fm_setowner_reply.status = 0;
	rsig->fm_setowner_reply.handle = htonl(handle);

	itc_send(&rsig, old_owner, ITC_MY_MBOX);
	return 0;
}

static int rcmd_sub_sig_req(union itc_msg *sig)
{
	union itc_msg *rsig;
...
static int rcmd_close_req(union itc_msg *sig)
{
	union itc_msg *rsig;
...
static int rcmd_set_pos_req(union itc_msg *sig)
{
	union itc_msg *rsig;
...
int main(int argc, char **argv)
{
   ...
   char cmd_argv[CMD_ARGV_MAX_SIZE] = "";
	...
	snprintf(lhsh_name, sizeof(lhsh_name), "lhsh_mbox_%p", (void*)(uintptr_t)getpid());
