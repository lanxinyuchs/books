...
void hdlc_trace_error(int line, const char *format, ...);
void hdlc_trace_info(int line, const char *format, ...);
void hdlc_trace_dbg(int line, const char *format, ...);
...
#define HDLC_ERROR(txt, ...) \
        hdlc_trace_error(__LINE__, txt, ##__VA_ARGS__)

#define HDLC_INFO(txt, ...) \
        hdlc_trace_info(__LINE__, txt, ##__VA_ARGS__)

#define HDLC_DBG(txt, ...) \
        hdlc_trace_dbg(__LINE__, txt, ##__VA_ARGS__)
...
#include <syslog.h>
#define HDLC_TRACE(...)
#define HDLC_ERROR(txt, ...) syslog(LOG_ERR, "[hdlc] " txt, ##__VA_ARGS__)
#define HDLC_INFO(txt, ...) syslog(LOG_INFO, "[hdlc] " txt, ##__VA_ARGS__)
#define HDLC_DBG(...)
#define HDLC_HEADER(...)
#define LNH_HEADER(...)
#endif
