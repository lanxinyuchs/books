...
union itc_msg {
	...
	struct a4ci_dataReqS			data_req;
	struct a4ci_dataCfmS			data_cfm;
	struct a4ci_dataRejS			data_rej;
};
...
static uint8_t a4ci_open(void)
{
	...
	msg->conn_req.protocolRev = htons(A4CI_PROTOCOL_REV);
			...
			uint32_t timeout, uint8_t *result)
{
	...
	msg = itc_alloc(sizeof(struct a4ci_dataReqS), A4CI_DATA_REQ);
	...
	msg = transact(msg, A4CI_DATA_CFM);
	*result = A4CI_SERVER_FAILURE;

	if (msg && (msg->msgno == A4CI_DATA_CFM)) {
...
static uint8_t do_test(uint8_t base_addr, uint16_t port)
{
	...
	for (i = 0; i < 2; i++) {
		...
		test = a4ci_datafwd(0, port, base_addr, 2, &data[0]);
		...
		do {
			...
			data[1] = CLIENT_IDENTIFIER;
			txtime = get_tick();
			res = a4ci_datareq(0, port, base_addr, 2, &data[0], TIMEOUTMS, &result);
			...
			if(res) {
				...
				LOG("CC_HW_PID_REQ Test failed with return value %d\n",result);
				passed = FAILED;

				if (result == A4CI_SERVER_TIME_OUT_NO_RESPONSE)
				{
					LOG("Result after time out of %dms\n"
		...
		if (!passed) {
			LOG("CC_HW_PID_REQ test for %d FAILED\n", base_addr);
			LOG("Link lost or Received Unexpected Data \n");
			return FAILED;
		}
		...
		do {
			...
			txtime = get_tick();
			res = a4ci_datareq(0, port, base_addr, 3, &data[0], TIMEOUTMS, &result);
			...
			if (res) {
				...
				elapse = (uint32_t)(get_tick() - txtime);
				LOG("CC_VISUAL_INDICATION_REQ Test failed with return value %d\n",result);
				passed = FAILED;
				if (result == A4CI_SERVER_TIME_OUT_NO_RESPONSE)
					LOG("Result after time out of %dms\n"
						"configured timeout is %dms\n", elapse, TIMEOUTMS);
		...
		if (!passed) {
			LOG("CC_VISUAL_INDICATION_REQ test for %d FAILED\n",base_addr);
			LOG("Link lost or Received Unexpected Data\n");
			return FAILED;
		}
		...
		do {
			...
			if (res) {
				...
				elapse = (uint32_t)(get_tick() - txtime);
				LOG("CC_CLEAR_START_FLAG_REQ Test failed with return value %d\n",result);
				passed = FAILED;
				if (result == A4CI_SERVER_TIME_OUT_NO_RESPONSE)
					LOG("Result after time out of %u ms\n"
		...
		if (!passed) {
			LOG("CC_CLEAR_START_FLAG_REQ test for %d FAILED\n",base_addr);
			LOG("Link lost or Received Unexpected Data \n");
			return FAILED;
		}
	}
...
static uint8_t do_test2(uint8_t base_addr, uint16_t port, uint32_t count)
{
	uint32_t i, failc = 0;
	uint8_t data[10] = {0};
	struct a4ciRx *res;
...
static uint8_t check_scu_device(uint32_t device)
{
	...
	res = a4ci_datareq(0, 1, device, 2, &data[0], TIMEOUTMS1, &result);
	if(res) {
...
static uint8_t check_sau_device(uint32_t device)
{
	...
	res = a4ci_datareq(0,2,device,2,&data[0],TIMEOUTMS1,&result);
...
static uint8_t bounds_test()
{
	...
	msg = itc_alloc(sizeof(msg->conn_req), A4CI_CONN_ESTABLISH_REQ);
	msg->conn_req.protocolRev = htons(A4CI_PROTOCOL_REV + 2);
	msg = transact(msg, A4CI_CONN_ESTABLISH_CFM);

	if (msg == NULL)
		return FAILED;

	if (msg->msgno != A4CI_CONN_ESTABLISH_REJ)
		goto exit_fail;

	if (ntohs(msg->conn_rej.errorCode) != A4CI_UNEXPECTED_PARAMETER_VALUE)
		goto exit_fail;

	if (ntohs(msg->conn_rej.protocolRev) != (A4CI_PROTOCOL_REV + 1))
		goto exit_fail;

	itc_free(&msg);

	msg = itc_alloc(sizeof(msg->data_req), A4CI_DATA_REQ);
	msg->data_req.port 		= htons(port); 
	msg->data_req.hdlcAddr 	= 0;
	msg->data_req.length 	= htons(64);
	...
	if (msg->msgno != A4CI_DATA_REJ)
		goto exit_fail;

	if (ntohs(msg->data_rej.errorCode) != A4CI_UNEXPECTED_PARAMETER_VALUE)
		goto exit_fail;

	if (ntohs(msg->data_rej.port) != port)
		goto exit_fail;

	if (msg->data_rej.hdlcAddr != 0)
		goto exit_fail;
	...
	msg = itc_alloc(sizeof(msg->data_req), A4CI_DATA_REQ);
	msg->data_req.port 		= htons(port); 
	msg->data_req.hdlcAddr 	= 126;
	msg->data_req.length 	= htons(80);
...
void *a4ci_thread(void *param)
{
	...
	uint16_t port = 1;
	uint8_t pass = FAILED;
	itc_mbox_id_t me, remote_peer, local_peer;
	int ret = 0;

	me = itc_create_mailbox("a4ci_thread", 0);
	if (me == ITC_NO_ID) {
	...
	if (addr == UNKNOWN_DEV) {
		LOG("No device of type %s found (%d)\n", td->pp_type, addr);
		ret = 3;
		goto exit;
	}

	local_peer = hunt_peer("a4ci_mcab_thread");
	msg = itc_alloc(sizeof(struct test_done), TEST_DONE);
	itc_send(&msg, local_peer, me);

	remote_peer = hunt_peer("ctr_link/g2_a4ci_thread");

	msg = itc_receive(select, 15000, ITC_FROM_ALL);
	if (msg == NULL) {
		LOG("No DONE signal from a4ci_mcab_thread\n");
		ret = 4;
		goto exit;
	}

	itc_send(&msg, remote_peer, me);
	msg = itc_receive(select, 15000, ITC_FROM_ALL);
	if (msg == NULL) {
		LOG("No DONE signal from g2_a4ci_thread\n");
		ret = 5;
		goto exit;
	}

	pass = do_test(addr, port);
	if (pass != PASSED) {
		LOG("CC_RESET test failed\n");
		ret = 6;
		goto exit;
	}
	...
	LOG("Doing CC_HWPID test. Number of iterations: %d\n", td->count);

	pass = do_test2(addr, port, td->count);
	if (pass != PASSED) {
