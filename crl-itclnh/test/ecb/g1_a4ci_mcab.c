...
static uint8_t dump_stats(uint16_t port)
{
	...
	if (msg == NULL) {
		LOG("No A4CI_LINK_STAT_CFM received\n");
		return FAILED;
	}
	if (ntohl(msg->link_cfm.clientRef) != cref) {
		LOG("Unexpected clientRef in LSTAT_CFM\n");
		return FAILED;
	}

	if (ntohs(msg->link_cfm.port) != port) {
		LOG("Unexpected port in LSTAT_CFM\n");
		return FAILED;
	}

	if (msg->link_cfm.nrOfUartOverrunsIsValid == 0) {
		LOG("ERROR - nrOfUartOverrunsIsValid: 0\n");
		return FAILED;
	}
...
void *a4ci_mcab_thread(void *param)
{
	...
	if (msg == NULL) {
		LOG("No DONE signal from a4ci_thread\n");
		ret = 2;
		goto exit;
	}
