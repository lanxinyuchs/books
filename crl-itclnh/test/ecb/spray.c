...
void *ping_thread(void *param)
{
	...
	for (count = 0; count < td->count; count++) {
		...
		if (msg == NULL) {
			LOG("No response from spray slave after %u (out of %u)"
					 "signals. Quitting.\n", count, td->count);
			td->result = 5;
			goto done;
		}
		...
		if ((t0 = (get_tick() - t0)) > tmax) {
			tmax = t0;
			LOG("New max RTD: %u milliseconds\n", (uint32_t)tmax);
		}
		...
		if (crcc != crcd) {
			LOG("CRC error in %zu byte message\n", itc_size(msg) - 8);
			td->result = 7;
			goto done;
		}

		if ((count % 100) == 0)
			LOG("Looped %d messages\n", count);
	...
	LOG("Spray test done... taking down link\n");
...
void *pong_thread(void *param)
{
	...
	for (;;) {
		...
		if (msg == NULL) {
			LOG("No messages from master in %u ms. Quitting.\n", rcv_tmo);
			goto done;
		}
		switch (msg->msgno) {
		case TEST_DATA:
		...
		case CLOSE_TEST:
			LOG("CLOSE_TEST received. %d messages looped\n", count);
			td->count = count;
			td->result = 0;
			itc_send(&msg, itc_sender(msg),ITC_MY_MBOX);
			usleep(10000);
		...
		default:
			LOG("Received unknown message 0x%x\n", msg->msgno);
			itc_free(&msg);
			break;
		}
	}
