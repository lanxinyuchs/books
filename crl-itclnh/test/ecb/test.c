...
void ecbt_print(const char *format, ...)
{
	va_list args;
	struct timeval tv;
	struct tm* ptm;
	...
	printf ("[%s.%03ld] %s", time_string, milliseconds, buffer);
}
...
static void *atfi_client(void *param)
{
	...
	if (msg->msgno == ATFI_CONN_ESTABLISH_REJ) {
		LOG("ATFI_CONN_ESTABLISH_REJ: error - %u\n", 
...
int main(int argc, char *argv[]) 
{
	...
	uint32_t addr = 4;
	uint32_t iter = 1000;
	itc_mbox_id_t mbox, peer, test_peer, ctrl_mbox;
	union itc_msg *msg;
	char *pp_type = NULL;
	int passed = 0;
	int master = 0;
	...
	if (!master) {
		...
		LOG("Done testing. Waiting for TEST_DONE from primary node\n");
		msg = itc_receive(sync, ITC_NO_TMO, ITC_FROM_ALL);
	...
	if (ret) {
		LOG("test_atfi_unsupported,  %d\n", ret);
		goto exit;
	}
	LOG("test_atfi_unsupported...: SUCCESS\n");

	ret = test_atfi_establish(peer);
	if (ret) {
		LOG("test_atfi_establish,  %d\n", ret);
		goto exit;
	}
	LOG("test_atfi_establish...: SUCCESS\n");

	ret = test_atfi_add_conn(peer);
	if (ret) {
		LOG("test_atfi_add_conn,  %d\n", ret);
		goto exit;
	}
	LOG("test_atfi_add_conn...: SUCCESS\n");

	ret = test_atfi_connect(peer);
	if (ret) {
		LOG("test_atfi_connect,  %d\n", ret);
		goto exit;
	}
	LOG("test_atfi_connec...: SUCCESS\n");

	ret = test_atfi_spray(peer, addr, count, size);
	if (ret) {
		LOG("test_atfi_spray,  %d\n", ret);
		goto exit;
	}
	LOG("test_atfi_spray...: SUCCESS\n");
	...
	LOG("Sending DONE to secondary node. Waiting for reply...\n");
	msg = itc_alloc(sizeof(struct test_done), TEST_DONE);
	itc_send(&msg, test_peer, ITC_MY_MBOX);

	msg = itc_receive(sync, 2*60000, ITC_FROM_ALL);
	if (msg) {
		if (msg->tdone.ecode == 0)
			passed = 1;
		else
			LOG("Error from secondary board: %d\n", msg->tdone.ecode);
	} else
		LOG("No response from secondary board.\n");


 exit:
	if (master) {
		msg = itc_alloc(sizeof(struct test_done), TEST_DONE);
		itc_send(&msg, ctrl_mbox, ITC_MY_MBOX);
	}

	LOG("Waiting for CTRL link to go down\n");
	...
	if (master)
		LOG("Primary QUIT (Test: %s)\n", passed ? "PASS" : "FAIL");
	else
		LOG("Secondary QUIT (Test: %s)\n", passed ? "FAIL" : "PASS");
