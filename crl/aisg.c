...
static void analyse_rsp_data(uint8_t *data, uint32_t size,
                             uint16_t *result,
                             uint8_t *device_type,
                     ...
                     uint8_t *rsp_uid_size)
{
	...
	ecb_unc_send(handle, NULL,
	             HDLC_ADDR_BC, HDLC_XID | HDLC_PF,
	             cmd_info, cmd_info_size);
...
int aisg_reset_device(void *handle, uint32_t timeout, uint8_t addr)
{
	...
	ecb_unc_send(handle, NULL,
	             addr, HDLC_XID | HDLC_PF,
	             cmd_info, sizeof(cmd_info));

	if (addr != HDLC_ADDR_BC) {
		(void) ecb_unc_wait(handle, NULL,
		                    addr,
		                    NULL, NULL, NULL,
		                    &rsp_tmo);
		usleep(MODEM_TXRX_TIME);
