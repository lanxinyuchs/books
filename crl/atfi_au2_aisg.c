...
static void fix_atma_workaround(struct connection *c)
{
	...
	do {
		...
		ecb_unc_send(au2_aisg.dev_handle, &c->stat, c->addr,
		             HDLC_RR | HDLC_PF | HDLC_VR(c->vr), 0, 0);

		res = ecb_unc_wait(au2_aisg.dev_handle, &c->stat, c->addr,
...
static void poll_snrm(struct connection *c)
{
	...
	/* Reset Device */
	aisg_reset_device(au2_aisg.dev_handle,
	...
	/* Address Assignment */
	res = aisg_address_assignment(au2_aisg.dev_handle,
	                              rsp_msec_tmo[c->bitrate_idx],
	                              c->addr,
	                              c->uid,
	                              c->uid_size,
...
static void poll_disc(struct connection *c)
{
	...
	if (ecb_dev_begin(au2_aisg.dev_handle) == 0) {
		...
		ecb_unc_wait(au2_aisg.dev_handle, &c->stat, c->addr,
		             &rsp_ctrl, 0, 0,
		             &rsp_nsec_tmo[c->bitrate_idx]);
		usleep(MODEM_RXTX_TIME);
		ecb_dev_end(au2_aisg.dev_handle);
...
static void handle_a2ci_reset_fwd(struct connection *c)
{
	...
	res = aisg_reset_device(au2_aisg.dev_handle,
	                        rsp_msec_tmo[c->bitrate_idx],
	                        c->addr);
	if (res != 0) {
