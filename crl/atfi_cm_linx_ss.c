...
static void* recv_thread(void *arg)
{
	...
	for (;;) {
		...
		res = 0;
		while (!cmd) {
			cmd = (uint8_t*) malloc(2 + cmd_info_size);
			if (!cmd) {
				if (!res) {
		...
		if (c->exit) { /* Time to exit. */
			...
			msg = itc_alloc(sizeof(struct ulh_transmsg_data),
			                ULH_TRANSMSG_DATA);
			msg->data.data.data = cmd;
			msg->data.data.size = cmd_info_size;
			msg->data.cid = 0;
...
static void reject_frame(struct connection *c, uint8_t ctrl)
{
	...
	ecb_unc_send(c->handle, &c->stat, c->addr,
	             HDLC_FRMR | HDLC_PF, &rsp_info, HDLC_FRMR_SIZE);
}
