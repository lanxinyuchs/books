...
do_disconnect(struct atf_conn *conn)
{
	...
	if (conn->tran_id != trans.alloc_cid) {
...
extern int check_for_dm(uint8_t addr, struct ulh_tbuff *tb);
static void send_dm_response(uint8_t addr, struct ulh_tbuff *tb)
{
	if (check_for_dm(addr, tb))
		rhai_ecb_write(trans.ecb_handle, tb->data, tb->size);
}
