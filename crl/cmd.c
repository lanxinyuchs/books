...
start_cmd_proc(int sock_fd, PROCESS shell_pid, int argc, char **argv)
{
   ...
   SIGSELECT     sel_die[] = {1, OS_ATTACH_SIG};
   union SIGNAL  *sig;
   PROCESS       pid;
   char          str[9];
   stdiofd_t     fds;
   ...
   else
   {
      ...
      if (sender(&sig) == shell_pid)
      {
        /*received attach from colish.wait for attach from the cmd_proc*/
        free_buf(&sig);
        sig = receive(sel_die);
        free_buf(&sig);
      }
...
shell_run_cmd(const char *cmd, const OSADDRESS *taglist)
{
   ...
   if ( !g_shell_parse_argv (cmd, &argp, &argv, &err) )
   {
   ...
   command = get_cmd(&lib_cmd_tree, argv[0]);
   if ( (command != NULL) && (command->func != NULL) )
   {
       status = command->func(argp, argv);
   }
   else
   {
      ptr = popen(cmd, "r");
