...
int ecb_dev_init(void **handle, const char *name)
{
	...
	if (stat(name, &st) == -1) {
		log_info(ctx->log_prefix, "stat(\"%s\") failed, errno: %d",
		        name, errno);
		goto err_open;
	}
	...
	ctx->stat_info = ecb_stat_create(name);
...
int ecb_dev_shutdown(void *handle)
{
	...
	if (ctx) {
		if (ctx->stat_info)
			ecb_stat_destroy(ctx->stat_info);
...
int ecb_dev_read(void *handle, void *data, uint32_t *size)
{
	...
	*size = len;
	...
	log_bus_receive(ctx->log_prefix, data, *size, "RX");
...
int ecb_dev_write(void *handle, const void *data, uint32_t size)
{
	...
	log_bus_send(ctx->log_prefix, data, size, "TX");
	if (write(ctx->dev.fd, data, (size_t) size) == -1) {
		log_info(ctx->log_prefix, "write() failed, errno: %d", errno);
		return -1;
	}

	return ecb_dev_drain(ctx);
}

static int ecb_rx(struct ecb_ctx *ctx, uint8_t term, ecb_dev_rx rx, void *obj,
                  uint8_t *data, uint32_t *size)
{
	uint8_t buffer[ECB_DEV_MRU];
	int     len, idx, ret;

	len = read(ctx->dev.fd, buffer, sizeof(buffer));
	if (len == -1) {
		log_info(ctx->log_prefix, "read() failed, errno: %d", errno);
		return -1;
	}
	log_bus_receive(ctx->log_prefix, buffer, len, "RX");
                    ...
                    struct timespec *const tmo)
{
	...
	do {
		...
		if (ret == -1) {
			...
			ret = ecb_rx(ctx, term, rx, obj, data, &size);
			if (ret != 0) {
				return ret;
			}
		}
...
int ecb_dev_flush(void *handle)
{
	...
	if (tcflush(ctx->dev.fd, TCIOFLUSH) == -1) {
		log_info(ctx->log_prefix, "tcflush() failed, errno: %d", errno);
		return -1;
	}
...
int ecb_dev_set(void *handle)
{
	...
	if (tcsetattr(ctx->dev.fd, TCSANOW, &ctx->dev.ts) == -1) {
		log_info(ctx->log_prefix, "ABN: tcsetattr() failed, errno: %d", errno);
		return -1;
	}
...
{
	struct ecb_ctx *ctx = (struct ecb_ctx*) handle;
	struct ecb_dev_stat_hdlc *stat;
	struct serial_icounter_struct *ser_counters;

	if (!ctx) {
		log_err(ECB_DEFAULT_PREFIX,
		        "ecb_dev_stat_opt() called with NULL pointer");
		return -1;
	}

	switch (option) {
	case ECB_DEV_STAT_OPT_UPDATE_HDLC:
	case ECB_DEV_STAT_OPT_READ_HDLC:
		if (!ctx->stat_info) {
			return -1;
		}

	case ECB_DEV_STAT_OPT_READ_UART:
		if (!value) {
			log_err(ctx->log_prefix, "bad parameter");
			return -1;
		}
		break;

	case ECB_DEV_STAT_OPT_RESET_HDLC:
		if (!ctx->stat_info) {
			return -1;
		}
		break;
	...
	default:
		log_trace2(ctx->log_prefix, "unknown option specified");
		return -1;
	}

	switch (option) {
	case ECB_DEV_STAT_OPT_UPDATE_HDLC:
		stat = (struct ecb_dev_stat_hdlc *)value;
		return ecb_stat_update(ctx->stat_info, stat);
	case ECB_DEV_STAT_OPT_READ_HDLC:
		stat = (struct ecb_dev_stat_hdlc *)value;
		return ecb_stat_read(ctx->stat_info, stat);
	case ECB_DEV_STAT_OPT_RESET_HDLC:
		return ecb_stat_reset(ctx->stat_info);
	case ECB_DEV_STAT_OPT_READ_UART:
		ser_counters = (struct serial_icounter_struct *)value;
		if (ioctl(ctx->dev.fd, TIOCGICOUNT, ser_counters) == -1) {
			log_info(ctx->log_prefix,
			         "Unable to fetch serial counters, errno: %d!",
			         errno);
			return -1;
		}
		return 0;
