...
static inline void ucc_update_stat(void *handle, struct ecb_ucc_stat *stat,
				   struct ecb_ucc_stat *local)
{
...
{
	struct ecb_ucc_stat local_stat;
	struct ucc_obj obj = {
		.addr = addr,
		.stat = &local_stat,
		.rsp_info_flag = 0,
	...
	memset(&local_stat, 0, sizeof(struct ecb_ucc_stat));
	...
	ucc_update_stat(handle, stat, &local_stat);
