                    ...
                    struct timespec *const tmo)
{
	...
	if (!handle) {
		log_err(ECB_DEFAULT_PREFIX, "ecb_unc_cmd_rsp() called with NULL pointer");
		return -1;
	}
                 ...
                 struct timespec *const tmo)
{
	...
	if (!handle) {
		log_err(ECB_DEFAULT_PREFIX, "ecb_unc_wait() called with NULL pointer");
		return -1;
	}
                 ...
                 uint8_t ctrl, void *info, uint32_t info_size)
{
	...
	if (!handle) {
                log_err(ECB_DEFAULT_PREFIX, "ecb_unc_send() called with NULL pointer");
		return -1;
	}
...
int ecb_unc_opt(void *handle, enum ecb_unc_opt option, const void *value)
{
	...
	if (!ctx) {
		log_err(ECB_DEFAULT_PREFIX, "ecb_unc_opt() called with NULL pointer");
		return -1;
	}
	...
	case ECB_UNC_OPT_N_OFLAGS:
		...
		if (u32 > ECB_UNC_OPT_MAX_OFLAGS) {
                       log_err(ctx->log_prefix, "ecb_unc_opt() invalid number of opening flags");
			return -1;
		}
	...
	case ECB_UNC_OPT_PREFIX:
		...
		if (u32 > ECB_UNC_OPT_MAX_PREFIX) {
                       log_err(ctx->log_prefix, "ecb_unc_opt() invalid number of "
			       "prefix characters");
			return -1;
		} else if (u32 == 0) {
			break;
		...
		if (!ctx->unc.prefix.data) {
                       log_err(ctx->log_prefix, "ecb_unc_opt() could not allocate space for "
			       "prefix characters");
			return -1;
		}
