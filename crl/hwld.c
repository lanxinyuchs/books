...
static int get_free_entry(void)
{
	...
	while (idx < mtd_info.size) {
		/*
		 * If read fails, we will try the next entry. The file position
		 * is not guaranteed in this case, so we set it manually.
		 */
		if (lseek(mtd_fd, idx, SEEK_SET) < 0)
			return -1;

		if (read(mtd_fd, &state, sizeof(state)) >= 0) {
			if (state == ADM_STATE_FREE)
				break;
		}
