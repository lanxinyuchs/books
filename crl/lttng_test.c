...
#define NO_OF_MULTIPLE_THREADS 100
#define NO_OF_THREAD_KILLS 100
#define NO_OF_REG_DEREG 100
...
DECLARE_TRACE_OBJ(LTTNG_TRI_OBJ_2);
...
OSENTRYPOINT(multiple_trace_thread);
OSENTRYPOINT(create_kill_trace_thread);
...
OS_PROCESS(lttng_tri_test)
{
     ...
     while(1)
     {
          ...
          case 1:
          {
               ...
               INFO(STR("Test of STR: Sianal name %s, Sianal No %d",
          ...
          case 15:
          {
               ...
               loops = sig_p->start.arg1;

               for (i = 0; i < loops; i++)
               {
                  thread_no = i;
                  pid1 = create_process(OS_PRI_PROC,
                                        "create_kill_trace_thread",
                                        create_kill_trace_thread,
                                        10000,
                                        (OSPRIORITY) 16,
                                        (OSTIME) 0,
          ...
          case 16:
          {
               ...
               for (i = 0; i < loops; i++)
               {
                  ...
                  deRegisterIfObj(&traceObjInfoLTTNG_TRI_OBJ_2);

                  sleep(2);

                  registerIfObj("LTTNG_TRI_OBJ_2", &traceObjInfoLTTNG_TRI_OBJ_2);
               }
               break;
          }
          case 17:
...
void printUsage()
{
     printf("Test command for test of tracing from Linux applications written in OSE style\n");
     printf("Usage: lttng_test <-tc n> [argc1] [args2]\n");
     ...
     printf("\t11: Generate LTTng traces from multiple threads at a high rate\n");
     ...
     printf("\t    arg2=Delay between loops (default: 1 sec)\n");
     printf("\t16: Register and deregister object\n");
     printf("\t    arg1=Number of loops (default: 10)\n");
     printf("\t    arg2=Delay between loops (default: 1 sec)\n");
     printf("\t17: Create multiple thread with same entry point but different names\n");
     printf("\t18: Generate traces with Low Level TRI functions\n");
}
...
int lttng_test_cmd(int argc, char **argv)
{
   int tc, arg1, arg2;
     ...
     if( (argc < 3) || (argc > 5) )
     {
          printf("Wrong number of arguments: argc=%d (3 > x < 5)\n", argc);
          printUsage();
          return -1;
     }
     ...
     if( argv[1][0] == '-' && argv[1][1] == 't' && argv[1][2] == 'c' )
     {
          ...
          if (argc == 4 || argc == 5)
          {
             arg1 = (U32)atoi(argv[3]);
             if( arg1 < 0 )
             {
                printUsage();
                return -1;
             }
             sig_p->start.arg1 = arg1;
          }
          if (argc == 5)
          {
             arg2 = (U32)atoi(argv[4]);
             if( arg2 < 0 )
             {
                printUsage();
                return -1;
             }
             sig_p->start.arg2 = arg2;
          }
...
OS_PROCESS(multiple_trace_thread)
{
   INFO(STR("Multiple Thread no %d", thread_no));

   while (1)
      sleep(100);
}

OS_PROCESS(create_kill_trace_thread)
{
   INFO(STR("Create and kill trace thread no %d procInfo %lu (%p)",
            thread_no, (unsigned long int)procInfo, procInfo));
...
OS_PROCESS(lttng_test)
{
     ...
     pid = create_process(OS_PRI_PROC,
                          "lttng_tri_no2_test_with_long_name",
                          lttng_tri_test,
                          10000,
                          (OSPRIORITY) 16,
                          (OSTIME) 0,
