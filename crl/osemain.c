...
#define MAX_PROCS(_no)				\
    ...
    if (setenv("MAX_PROCS", __buf_mp, 1) < 0)	\
	return -1;
#include <osemain.con>
#undef MAX_PROCS
#define MAX_PROCS(_no)
...
#define OSE_LM_POOL_SIZE(_sz)			      \
    ...
    if (setenv("OSE_LM_POOL_SIZE", __buf_ps, 1) < 0)  \
	return -1;
#include <osemain.con>
#undef OSE_LM_POOL_SIZE
#define OSE_LM_POOL_SIZE(_sz)
...
#define OSE_LM_SIGNAL_SIZES(_x0, _x1, _x2, _x3, _x4, _x5, _x6, _x7)	\
    ...
    if (setenv("OSE_LM_SIGNAL_SIZES", __sig_szs, 1) < 0)		\
	return -1;
#include <osemain.con>
#undef OSE_LM_SIGNAL_SIZES
#define OSE_LM_SIGNAL_SIZES(_x0, _x1, _x2, _x3, _x4, _x5, _x6, _x7)
...
/** ==================================================================== */
/** 
 *   Dummy main function or not no function depending on how USER_START
 *   is configured.
 * 
 *   @param argc       Standard main argc (not used)
 *   @param argv       Standard main argv (not used)
 * 
 *   @return           0
 *
 *   @par Globals:     
 ...
 *
 *   The dummy main function is never called. It is only included to make
 *   the linker happy.
 */
/* ===================================================================== */
#undef USER_START
...
/** ==================================================================== */
/** 
 *   Process that runs the main() function.
 * 
 *   @param            -
 * 
 *   @return           -
 ...
 *                     --
 *
 *   If USER_START is set to YES, the main() function in the application
 *   code is run. Otherwise main() is just a dummy (dummy_main()) that
 *   only stops this process.
 *
 *   Prototype: USER_START ( YES | NO )
 */
/* ===================================================================== */
OS_PROCESS(main_proc)
{
  int argc;
  char **argv;

  argc = strtoul(getenv("ARGC"), NULL, 0);
  argv = (char **)strtoul(getenv("ARGV"), NULL, 0);
 ...
 *                     --
 *
 *   Starts a new process that actually runs the main() function.
 */
/* ===================================================================== */
static int
start_main(int argc, char **argv)
{
  ...
  sprintf(str, "%lu", (unsigned long)argc);
  setenv("ARGC", str, 1);
  sprintf(str, "%lu", (unsigned long)argv);
  setenv("ARGV", str, 1);

  strcpy(str, "main_proc_");
  strncat(str, basename(argv[0]), 64 - strlen("main_proc_") - 1);

  start(create_process(OS_PRI_PROC, str, main_proc, 2000, 0, (OSTIME)0, 0,0, 0, 0));
  return 0;
}

/** ==================================================================== */
/** 
 *   Real main function or a dummy main function depending on how
 *   USER_START is configured.
 * 
 *   @param argc       Standard main argc (not used)
 *   @param argv       Standard main argv (not used)
 * 
...
static void
start_osemain_stuff(int argc, char **argv)
{
#undef USER_START
#define USER_START(val) val
#define	YES					\
...
__wrap_main(int argc, char **argv)
{
  ...
  run_hook1();
  start_osemain_stuff(argc, argv);
  return zzos_main(argc, argv);
}
