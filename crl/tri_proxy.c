 ...
 * Get the thread name.
 */
#define GET_THREAD_NAME(param)   prctl(PR_GET_NAME,param,0,0,0)
 ...
 *   the envrionment varibles for ITC_NAMESPACE which was intiated
 *   
 *
...
initPriProc(const char *procName)
{
   ...
   *procInfo = defaultItemInfo;
   ...
   if ((ret = GET_THREAD_NAME((unsigned long)tid_name)) == 0)
   {
     instanceId = getInstanceId(getpid());
     if (instanceId != 0)
      {
         snprintf(procInfo->procName, TRI_MAX_PROC_NAME_LEN, "%d/%s",
                  instanceId, tid_name);
      }
      else
      {
         snprintf(procInfo->procName, TRI_MAX_PROC_NAME_LEN, "%s",
                  tid_name);
      }
   }
   else
   {
      syslog(LOG_ERR, "TRI proxy %s: failed to get thread name for "
             "%s (%d), errno=%d.", __FUNCTION__, procName, ret, errno);  
   }
...
deRegisterIfObj(struct ItemInfo **ref)
{
   ...
   msg->triDaemonDeregisterItemReq.itemType = TRI_ITEMTYPE_IFOBJ;
   ...
   /* Free the trace item struct memory. ItemPtr1 or ItemPtr2 points
    * to the same pointer as referenced by ref.
    */
   free(*ref);
}
 ...
 *
 *
  *   @param traceObj_p   Reference to interface and object information struct
 *
 *   @return           -
 */
/* ===================================================================== */
 ...
 *
 *   @param ref        Reference to interface and object information struct.
 *                     If set to NULL, this indicates that two pointers is
 *                     provided to the trace item struct.
 *
 *   @return           -
 *
 *   @par Globals:
               ...
               ...)
{
    ...
    * struct when 'ref' argunebt is set to NULL. Otherwise the
   ...
   *(*ref) = defaultItemInfo;
    ...
    *  when deregister the trace item. Store first provided
    *  pointer in itemPtr1.
    */
    ...
    * Now either of the refernces can be used for trace logging.
    * Store the second pointer address in the struct.
    */
   if (ref2)
   {
     *ref2 = *ref;
