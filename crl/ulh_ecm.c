...
static int ecm_rx_data(struct ulh_ecm_conn *co, struct ulh_tbuff *tb)
{
	...
	if (!q) {
		ULH_TRACE_INFO("ECM: %s: unable to alloc rxTB", co->name);
		ecm_disconnect(co, 1, __FUNCTION__, __LINE__);
		return ECM_LAST;
	}
			   ...
			   struct ulh_cm_msghdr *cmhdr, union itc_msg *msg)
{
	...
	if (!tb) {
           ...
           ULH_TRACE_INFO("DBG: ECM: %s: unable to allocate tb",
			       co->name);
	...
	if (ulh_tbuff_alloc(co->cid, frag_size + hdr_size, tb)) {
		ULH_TRACE_INFO("%s: unable to allocate packet", co->name);
		ecm_disconnect(co, 1, __FUNCTION__, __LINE__);
		return -ENOMEM;
	}
	...
	while(size) {
		...
		tb = ulh_tbuff_pool_get(&co->tbpool);
		if (!tb) {
			ULH_TRACE_INFO("ECM: %s: unable to allocate tb",
				       co->name);
			ecm_disconnect(co, 1, __FUNCTION__, __LINE__);
			return -ENOMEM;
		}
