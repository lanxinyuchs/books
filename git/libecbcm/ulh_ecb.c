...
int _ulh_trans_transmit(struct conn_object *co, struct ulh_tbuff *tb)
{
	...
	return ulh_trans_transmit(co->cid, tb);
}
...
static int check_frame(struct conn_object *co,  struct ulh_tbuff* skb)
{
	...
	for (ulh_tbuff_hold(skb),co->rx_frags[seq] = skb; co->rx_frags[co->vr];) {
		...
		if (hdr & ECB_FRAG_F_HEADER) {
			...
			if (co->rx_current || (skb->size <= ECB_FRAG_SF_HDRSIZE)) {
				ECB_ERROR("[%s] Ffrag current: %p  len %d\n", co->name,
			...
			if (skb->size > co->rx_hdr.size) {
				ECB_ERROR("[%s] Ffrag error (hdr %d frag %d)\n", co->name,
			...
			if ((co->rx_offset + skb->size) > co->rx_hdr.size) {
				ECB_ERROR("[%s] Mfrag error (hdr %d actual %d)\n", co->name,
			...
			ECB_TRACE("[%s] Mfrag current: %p  len %d\n", co->name,
									(void*)co->rx_current,skb->size);
			goto eout;
		}
		...
		if (hdr & ECB_FRAG_L_HEADER) {
			...
			co->uc.uc_delivery(co->lobj, &co->rx_hdr, co->rx_current);
			co->rx_current = NULL;
        }
    }
...
static void poll_task(void *data)
{
	...
	if (!ebus_busy) {
		...
		ulh_timer_arm(&co->polltmo, co->tqueue, HDLC_REPOLL_TMO);
		return;
	}
...
{
	if (ebus_busy) {
		ECB_DEBUG("Poll bit lost. Releasing bus after timeout.");
		ebus_busy = 0;
		pthread_mutex_unlock(&ebus_lock);
	}
}
...
{
    struct conn_object *co  = (struct conn_object *)cobj;
    __disconnect(co, NDM, DISC_ORDERED, 1, __LINE__);
                               ...
                               struct ulh_timerqueue *tqueue)
{
	...
	ulh_timer_init(&co->polltmo, poll_task, co);
