...
static void mhp_tx_ctrl(struct mhp_link *co, uint8_t type)
{
	...
	tx_send(co, tb);
	tx_free(tb);
	pthread_mutex_unlock(&co->tx_lock);
}
...
static void mhp_tx(struct mhp_link *co)
{
	...
	if (pthread_mutex_trylock(&co->tx_lock))
		return;
...
int mhp_init(struct mhp_link *mhp)
{
	mhp->tx_wnd   = 8;
	mhp_reset_link(mhp);
	pthread_mutex_init(&mhp->tx_lock, NULL);
	pthread_mutex_init(&mhp->ep_lock, NULL);
	pthread_mutex_init(&mhp->lc_lock, NULL);
...
void *mhp_recv(void *param)
{
	...
	me = itc_create_mailbox("mhp_recv", 0);
	if (me == ITC_NO_ID) {
	...
	for (;;) {
		...
		if (len < (sizeof(struct ether_hdr) + sizeof(struct mhp_header))) {
			MDU_ERROR("discarding packet of len %d (%d)", len, -errno);
			continue;
		}
