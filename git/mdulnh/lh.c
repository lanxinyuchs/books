...
{
	pthread_mutex_lock(&g.htab_lock);
	if (hash < HTAB_SIZE) {
		htab[hash] = 0;
		update_bmap(hash, 0);
	}
	pthread_mutex_unlock(&g.htab_lock);
}
...
static struct mhp_locate *find_locate(struct mhp_link *link, char *name)
{
	...
	pthread_mutex_lock(&link->lc_lock);
	...
	pthread_mutex_unlock(&link->lc_lock);
	return q;
}
...
{
	link = rep->link;
	pthread_mutex_lock(&g.endp_lock);
	htab_del(rep->hash);
	itc_delete_mailbox(rep->mbox);
	link->rep_table[rep->addr] = 0;
	free(rep->aptr);
	pthread_mutex_unlock(&g.endp_lock);
}
							...
							itc_mbox_id_t mbox)
{
	...
	pthread_mutex_lock(&link->ep_lock);
	*lepp = lep = htab_get(mbox, HTAB_TYPE_LEP);
	...
	pthread_mutex_unlock(&link->ep_lock);
	return ret;
}
												...
												union itc_msg* msg)
{
	...
	if (rep) {
		MDU_ERROR("[MDU]: msgno 0x%x from 0x%x to unknown dst_addr %d",
...
static void handle_lep_dead(union itc_msg *msg)
{
	...
	if (lep) {
		pthread_mutex_lock(&g.link_lock);
		free_lep(lep,NULL);
		pthread_mutex_unlock(&g.link_lock);
	}
}
...
{
	uint32_t select[2] = {1, MDU_CREATE_REQ};
	...
	pthread_mutex_init(&g.link_lock, &mattr);

	pthread_mutexattr_init(&mattr);
	pthread_mutexattr_settype(&mattr, PTHREAD_MUTEX_ADAPTIVE_NP);
	pthread_mutex_init(&g.endp_lock, &mattr);
	pthread_mutex_init(&g.htab_lock, &mattr);
	...
	if (pthread_create(&g.rx_thread, NULL, mhp_recv, NULL)) {
		MDU_ERROR("pthread_create failed, %d", -errno);
		goto eout;
	}
	g.ns_mbox = ITC_NO_ID;
	...
	for (;;) {
		msg = itc_receive(ITC_NOFILTER, 5, ITC_FROM_ALL);
		if (msg) {
