...
static int test_link_create(struct test_args *targ)
{
	...
	for (count = 0; count < 100; count++) {
		...
		/* 63 links should be available currently */
...
void *test_prog_m(void *ctx)
{
	...
	ret = rolc_link_setup(handle, "slave", &cfg, &link_id);
	if (ret) {
		printf("rolc_link_setup, %d\n", ret);
		goto get_out;
	}

	itc_locate_async("slave/rolc_test_slave", NULL, ITC_MY_MBOX);
