/* ----------------------------------------------------------------------
 * %CCaseFile:	add_board.c %
 * %CCaseRev:	/main/R8A/2 %
 * %CCaseDate:	2016-12-22 %
 * %CCaseDocNo: %
...
void usage(void)
{
  printf("usage: add_board -b boardtype -r boardrev\n");
}
...
int main(int argc, char *argv[])
{
  ...
  Result = Lmhi_add_board(boardtype,boardrev,mboxid);
  ...
  if (Result == LMHI_WAIT) {
    ...
    switch(msg->msgno) {
    case LMHI_ADD_BOARD_RSP:
      printf("LMHI_ADD_BOARD_RSP received with\n");
      printf("BoardNo = %s \n",msg->LmhiAddBoardRsp.BoardNo);
      printf("BoardRev = %s\n",msg->LmhiAddBoardRsp.BoardRev);
      break;
    case LMHI_ADD_BOARD_REJ:
      printf("LMHI_ADD_BOARD_REJ received\n");
      printf("BoardNo = %s \n",msg->LmhiAddBoardRej.BoardNo);
      printf("BoardRev = %s \n",msg->LmhiAddBoardRej.BoardRev);
      printf("RejReason = %s \n",msg->LmhiAddBoardRej.RejReason);
      break;
