...
void usage(void)
{
  printf("usage: delete_ext_board -b boardtype -r boardrev\n");
}
...
int main(int argc, char *argv[])
{
  ...
  mboxid = itc_create_mailbox("DELETE_EXT_BOARD",0);

  Result = Lmhi_delete_ext_board(boardtype, boardrev, mboxid);
  ...
  if (Result == LMHI_WAIT) {
    ...
    switch(msg->msgno) {
    case LMHI_DELETE_EXT_BOARD_RSP:
      printf("LMHI_DELETE_EXT_BOARD_RSP received with\n");
