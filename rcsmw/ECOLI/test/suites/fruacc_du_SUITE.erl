%%% #0.    BASIC INFORMATION
%%% ----------------------------------------------------------
%%% %CCaseFile:	fruacc_du_SUITE.erl %
%%% @author uabesvi
%%% @copyright Ericsson AB 2015-2016
%%% @version /main/R4A/R8A/1
-module(fruacc_du_SUITE).
-id('Updated by CCase').
-vsn('/main/R4A/R8A/1').
-date('2016-11-16').
-author('uabesvi').
...
-define(DU2,  "ManagedElement=1,Equipment=1,FieldReplaceableUnit=Mp2").
...
init_per_suite(Config) -> 
    ok = start_du(1, list_to_binary(?DU1), primary),
    ok = start_du(3, list_to_binary(?DU2), undefined),
    Config.
%% @hidden
...
%%--------------------------------------------------------------------
tab_fru(_) ->
    send("ManagedElement=1,Equipment=1,FieldReplaceableUnit=3\t", ?RU2),
    send("\t", ?RU2).
...
start_du(DU, Name, Type) ->
    du_name(DU, Name, Type),
    sd_wait(fruacc_lib:rpc(clhI, op_state, [DU]), 60, DU).

sd_wait(enabled, _, _) ->
    ok;
sd_wait(_, N, DU) when N < 0 ->
    {error, {du_not_started, DU}};
sd_wait(_, N, DU) ->
    timer:sleep(1000),
    sd_wait(fruacc_lib:rpc(clhI, op_state, [DU]), N-1, DU).
