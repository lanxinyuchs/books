...
%%% Rev        Date       Name        What
%%% -----      ---------  --------    ------------------------
%%% R1A/3      2012-04-11 etxpeno     Added Reason 'liciMissingFile' in
%%%                                   verifyLicenseFile/1
%%% ----------------------------------------------------------
%%%
%%% #2.    EXPORT LISTS
...
-spec verifyLicenseFile(FileName) -> Result when
      FileName :: string(),
      Result :: 'ok' | {'error', Reason},
      Reason :: 'liciMissingFile' | 'liciBadFile' | 'liciVfyFailed'.

verifyLicenseFile(FileName) ->
    try
	LicenseData = get_file(FileName),

	ProdCert = get_prodcert(LicenseData),
	CaCert = get_cacert(LicenseData),

	PublicKey = validate_path([CaCert, ProdCert]),

	Body = get_body(LicenseData),
	Signature = get_signature(LicenseData),
	verify_signature(Body, Signature, PublicKey),
...
get_file(FileName) ->
	...
	{ok, LicenseData} ->
	    LicenseData;
	{error, _Reason} ->
	    throw(liciMissingFile)
    end.

get_prodcert(LicenseData) ->
    get_cert(LicenseData, "prodcert").

get_cacert(LicenseData) ->
    get_cert(LicenseData, "cacert").
...
get_cert(LicenseData, Tag) ->
	...
	try
	    ...
	    PemStart = Start0+Length0,
	    {Start1, _} = binary:match(LicenseData, EndTag,
				       [{scope, {PemStart, byte_size(LicenseData)-PemStart}}]),
	    PemEnd = Start1-1,
	    binary:part(LicenseData, {PemStart, PemEnd-PemStart})
	catch
	    _:_ ->
		throw(liciBadFile)
	end,
...
validate_path(CertList) ->
	...
	{ok, {{_Algorithm, PublicKey, _PublicKeyParams}, _PolicyTree}} =
	    public_key:pkix_path_validation(rootCert(),
					    CertList,
					    [{verify_fun, validationFunState()}]),
...
get_signature(LicenseData) ->
	...
	{Start2, _} = binary:match(LicenseData, <<"</PKIsignature>">>,
				   [{scope, {Start1, Size-Start1}}]),
	PemEnd = Start2-1,
	base64:decode(binary:part(LicenseData, {PemStart, PemEnd-PemStart}))
    catch
	_:_ ->
	    throw(liciBadFile)
    end.
...
verify_signature(Body, Signature, PublicKey) ->
    try
	true = public_key:verify(Body, 'sha', Signature, PublicKey)
    catch
	_:_ ->
	    throw(liciVfyFailed)
    end.
...
-ifdef(TESTCERT).
rootCert() ->
    %% This is the root certificate used in testing purposes
    <<16#30,16#82,16#02,16#29,16#30,16#82,16#01,16#92,16#a0,16#03,16#02,16#01,
      16#02,16#02,16#01,16#00,16#30,16#0d,16#06,16#09,16#2a,16#86,16#48,16#86,
      16#f7,16#0d,16#01,16#01,16#05,16#05,16#00,16#30,16#53,16#31,16#0b,16#30,
      16#09,16#06,16#03,16#55,16#04,16#06,16#13,16#02,16#53,16#45,16#31,16#14,
      16#1c,16#ae,16#1a,16#6c,16#e8,16#f6,16#51,16#47,16#94,16#98,16#d5,16#0f,
      16#23,16#f9,16#0d,16#6a,16#f0,16#40,16#93,16#19,16#57,16#d3,16#7c,16#86,
      16#72,16#44,16#89,16#c7,16#86,16#53,16#a6,16#f5,16#5f,16#94,16#fb,16#93,
      16#db,16#0a,16#66,16#7a,16#92,16#99,16#29,16#31,16#c9,16#99,16#61,16#eb,
      16#f6,16#f5,16#51,16#35,16#9a,16#0d,16#02,16#03,16#01,16#00,16#01,16#a3,
      16#0d,16#30,16#0b,16#30,16#09,16#06,16#03,16#55,16#1d,16#13,16#04,16#02,
      16#30,16#00,16#30,16#0d,16#06,16#09,16#2a,16#86,16#48,16#86,16#f7,16#0d,
      16#01,16#01,16#05,16#05,16#00,16#03,16#81,16#81,16#00,16#a7,16#5f,16#fb,
      16#1f,16#91,16#f7,16#15,16#6e,16#45,16#4c,16#5d,16#48,16#d4,16#03,16#46,
      16#d3,16#52,16#eb,16#6e,16#a3,16#b1,16#c6,16#d8,16#54,16#de,16#1c,16#86,
      16#99,16#39,16#6a,16#5b,16#64,16#2f,16#9a,16#e8,16#15,16#da,16#01,16#e8,
      16#69,16#fe,16#85,16#35,16#9c,16#7e,16#11,16#7c,16#78,16#fb,16#63,16#f9,
      16#96,16#d1,16#f3,16#2a,16#ce,16#e5,16#df,16#e7,16#7c,16#3d,16#a3,16#35,
      16#e5,16#6c,16#a7,16#7c,16#4d,16#55,16#54,16#70,16#16,16#50,16#1a,16#4c,
      16#ba,16#1b,16#2e,16#2a,16#6f,16#f5,16#9a,16#94,16#ec,16#6e,16#18,16#f2,
      16#8c,16#a7,16#b7,16#96,16#20,16#96,16#4a,16#b8,16#f6,16#64,16#03,16#21,
      16#d1,16#2f,16#f5,16#9b,16#c5,16#fb,16#cb,16#47,16#54,16#25,16#dc,16#30,
      16#bd,16#15,16#96,16#93,16#20,16#c2,16#0a,16#9b,16#30,16#e3,16#e0,16#17,
      16#90,16#89,16#77,16#dd,16#7f>>.
-else.
...
validationFunState() ->
    Fun = fun(_, {bad_cert, missing_basic_constraint}, UserState) ->
		  %% The Basic Constrains extension is not mandatory
		  {valid, UserState};
	     (_, {bad_cert, _} = Reason, _) ->
		  {fail, Reason};
	     (_, {extension, #'Extension'{extnID = ?'id-ce-subjectKeyIdentifier'}}, UserState) ->
		  {valid, UserState};
	     (_, {extension, #'Extension'{extnID = ?'id-ce-basicConstraints'}}, UserState) ->
		  {valid, UserState};
	     (_, {extension, #'Extension'{extnID = ?'id-ce-nameConstraints'}}, UserState) ->
		  {valid, UserState};
