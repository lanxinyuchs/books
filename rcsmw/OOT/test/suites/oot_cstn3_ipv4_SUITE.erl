%%% #0.    BASIC INFORMATION
%%% ----------------------------------------------------------
%%% %CCaseFile:	oot_cstn3_ipv4_SUITE.erl %
%%% @author eolaand
...
%%% ----------------------------------------------------------
%%% Rev        Date       Name        What
%%% R11A/1     2017-09-12 eolaand     Created
%%% ----------------------------------------------------------

-module(oot_cstn3_ipv4_SUITE).
     ...
     {ct_hooks, [{rct_rpc, ?TNODE},
		 %% {rct_snmpmgr, snmp1},
		  ...
		  {rabbe_logs, [{erlang,{["ERROR REPORT","CRASH REPORT"],[]
                 ...
                 {rct_proxy, 
		  [{node_data, [{1, ?IFT_NODE, ssh_lmt_ipv4, du1, username}]}, 
		   {opts, [{restart_on_fail, false}]}]},
                 {rct_core, []},
...
end_per_testcase(_TestCase, _Config) ->
    LDNs = [?LDN_IPV4_1, ?LDN_IPV4_2, ?LDN_IPV4_3],
...
set_and_delete_alt_access_point(_Config) ->
    LdnAccPoint = ?LDN_IPV4_1,
    LdnAltAccPoint = ?LDN_IPV4_2,

    IP = ?TN_GW,
    IPAddr = IP ++ ?TN_MASK,
    IPAlt = ?TN_GW_ALT,
    IPAddrAlt = IPAlt ++ ?TN_MASK_ALT,
...
ip_change_cb_one_ldn(_Config) ->    
    ...
    LdnV4 = ?LDN_IPV4_3,
    IPAddr = ?TN_FH_1 ++ ?TN_MASK,
    IPUpdate = ?TN_FH_2 ++ ?TN_MASK,
    %% Subscribe to IP updates in OOT
    ok = register_ip_change_cb(LdnV4),
    ok = send_proxy(?SetOamDNtoNsNameRsp, {LdnV4, ?FH_NS, IPAddr}),

    %% Run get_ns_and_ip to emulate subscription in TN
    {ok, ?FH_NS, IPAddr} = get_ns_and_ip(LdnV4),
    
    %% Check with IFT that a lookup was done
    {ok, {cstnDnToTn, {LdnV4, {?FH_NS, IPAddr}}}} = receive_proxy(),

    %% Update IP Address    
    ok = send_proxy(?CsTnUpdate, {LdnV4, ?FH_NS, IPUpdate}),

    %% Check that we get a notification
    ok = receive_ip_changed({LdnV4, ?FH_NS, IPUpdate}),

    %% Update back to orig IP Address    
    ok = send_proxy(?CsTnUpdate, {LdnV4, ?FH_NS, IPAddr}),

    %% Check that we get a notification
    ok = receive_ip_changed({LdnV4, ?FH_NS, IPAddr}),
...
ip_change_cb_two_ldn(_Config) ->    
    ...
    Ldn1 = ?LDN_IPV4_2,
    Ldn2 = ?LDN_IPV4_3,
    IPAddr1 = ?TN_FH_1 ++ ?TN_MASK,
    IPUpdate1 = ?TN_FH_2 ++ ?TN_MASK,
    IPAddr2 = ?TN_FH_3 ++ ?TN_MASK,
    IPUpdate2 = ?TN_FH_4 ++ ?TN_MASK,

    %% Subscribe to IP updates in OOT for the first LDN
    ok = register_ip_change_cb(Ldn1),
    ok = send_proxy(?SetOamDNtoNsNameRsp, {Ldn1, ?FH_NS, IPAddr1}),

    %% Run get_ns_and_ip to emulate subscription in TN
    {ok, ?FH_NS, IPAddr1} = get_ns_and_ip(Ldn1),
    
    %% Check with IFT that a lookup was done
    {ok, {cstnDnToTn, {Ldn1, {?FH_NS, IPAddr1}}}} = receive_proxy(),

    %% Subscribe to IP updates in OOT for the second LDN
    ok = register_ip_change_cb(Ldn2),
    ok = send_proxy(?SetOamDNtoNsNameRsp, {Ldn2, ?FH_NS, IPAddr2}),

    %% Run get_ns_and_ip to emulate subscription in TN
    {ok, ?FH_NS, IPAddr2} = get_ns_and_ip(Ldn2),
    
    %% Check with IFT that a lookup was done
    {ok, {cstnDnToTn, {Ldn2, {?FH_NS, IPAddr2}}}} = receive_proxy(),

    %% Update IP Address of the first LDN    
    ok = send_proxy(?CsTnUpdate, {Ldn1, ?FH_NS, IPUpdate1}),

    %% Check that we get a notification
    ok = receive_ip_changed({Ldn1, ?FH_NS, IPUpdate1}),

    %% Update IP Address of the second LDN    
    ok = send_proxy(?CsTnUpdate, {Ldn2, ?FH_NS, IPUpdate2}),

    %% Check that we get a notification
    ok = receive_ip_changed({Ldn2, ?FH_NS, IPUpdate2}),

    %% Update first LDN back to orig IP Address    
    ok = send_proxy(?CsTnUpdate, {Ldn1, ?FH_NS, IPAddr1}),

    %% Check that we get a notification
    ok = receive_ip_changed({Ldn1, ?FH_NS, IPAddr1}),

    %% Update second LDN back to orig IP Address    
    ok = send_proxy(?CsTnUpdate, {Ldn2, ?FH_NS, IPAddr2}),

    %% Check that we get a notification
    ok = receive_ip_changed({Ldn2, ?FH_NS, IPAddr2}),
...
change_access_point(_Config) ->    
    LdnAccPoint = ?LDN_IPV4_1,
    LdnAccPointNew = ?LDN_IPV4_2,

    IP = ?TN_GW,
    IPAddr = IP ++ ?TN_MASK,
    IPNew = ?TN_GW_NEW,
    IPAddrNew = IPNew ++ ?TN_MASK,
...
%%--------------------------------------------------------------------
%% @doc
%% Set primary and alt AP, and register a subscriber to OOT config updates. 
%% First change IP of primary OAP and check notifications.
%% Then set primary OAP back to old IP and change IP of Alt OAP to the 
%% same IP that was used to update the primary OAP.
%% Then revert also the Alt OAP back to previous. <br/>
%% @end
%%--------------------------------------------------------------------
update_alt_access_point_ip(_Config) ->    
    ...
    ct:pal("Update accessPoint IP from ~p to ~p~n", [IPNew, IP]),
...
%%     ?LIB:create_ipv4_instances().


%% get_new_ipv4_addr() ->
%%     ?LIB:get_new_ipv4_addr().
