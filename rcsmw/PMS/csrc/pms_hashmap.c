 ...
 * R2A/6      2013-04-10 erarafo     Using 'const' specifiers
 * R2A/8      2013-05-03 erarafo     Interface functions return codes
 * R2A/9      2014-01-20 erarafo     Using 'const char'
 * ----------------------------------------------------------------------
 */
		...
		int k; for (k = 0; k < numberOfApps; k++) {
			if (t >= apps[k].delay) {
				...
				if (u == 0) {
					...
					if (threadUnsafePut(key, apps[k].name) != PMS_HASHMAP_OK) {
						return false;
					}
					apps[k].handle = key;
#if SELF_TEST_VERBOSE
	...
	if (usedKeys != NULL) {
		*result = usedKeys->key;
		...
		if (asprintf((char **)result, "%s-%d", prefix, keyNumber) == -1) {
			return PMS_HASHMAP_OUT_OF_MEMORY;
		}
		else {
			return PMS_HASHMAP_OK;
		}
...
int threadUnsafePut(char const *key, void const *value) {
	struct hsearch_data * const map = getMap();
	if (map == NULL) {
		return PMS_HASHMAP_UNSPECIFIC_FAILURE;
	}
	...
	if (hsearch_r(wrappedKeyValue, FIND, &result, map) == 0) {
		// no such key yet
	        if (entryCount == PMS_HASHMAP_SIZE) {
	            return PMS_HASHMAP_MAX_KEYS_EXCEEDED;
	        }
...
int pmiMap_clear(char const * key) {
	...
	if (map == NULL) {
		...
		if (hsearch_r(wrappedKey, FIND, &result, map) == 0) {
			...
			LOCK_UNLOCK(&map_lock);
			return PMS_HASHMAP_CLEAR_NONEXISTENT_ENTRY;
		}
		else if (result->data == NULL) {
			// trying to clear an entry that was already cleared
			LOCK_UNLOCK(&map_lock);
			return PMS_HASHMAP_CLEAR_ENTRY_TWICE;
		}
		else {
			result->data = NULL;
...
struct hsearch_data *getMap() {
	if (! isMapInitialized) {
		if (hcreate_r(PMS_HASHMAP_SIZE, &map) == 0) {
			return NULL;
		}
