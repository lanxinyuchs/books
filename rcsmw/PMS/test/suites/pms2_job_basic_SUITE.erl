...
%%% #0.    BASIC INFORMATION
%%% ----------------------------------------------------------
%%% %CCaseFile:	pms2_job_basic_SUITE.erl %
...
%%% Rev        Date       Name        What
%%% -----      ---------  --------    ------------------------
%%% R3A/1      2014-09-04 uabesvi     Copied from pms_job_basic_SUITE
...
-export([same_app_sequnce_jobs/1]).
...
session_two_apps(_Config) ->
    ...
    {ok, Sune} = initialize(?SUNE, ?CB_DEF),
    {ok, Tina} = initialize(?TINA, ?CB_DEF),
...
counter_maps(_Config) ->
    ...
    %% send counter_map
    %%-------------------------------------------------------------
    counter_map(?SUNE, Sune, [{"Group1", 1, [{"Type1", 1}]}]),    
    counter_map(?SUNE, Sune, [{"Group2", 2, [{"Type2", 34}]}]),    
    to(),
    ...
    true = lists:member({1, <<"Group1">>}, Aliases),
    true = lists:member({2, <<"Group2">>}, Aliases),
    true = lists:member({{1, 1},  <<"Type1">>}, Aliases),
    true = lists:member({{2, 34}, <<"Type2">>}, Aliases),
...
counter_maps_redefine(_Config) ->
    ...
    %% test multi MTs
    %%-------------------------------------------------------------
    counter_map(?SUNE, Handle, [{"Group1", 1, [{"Type1", 1}]}]),    

    counter_map(?SUNE, Handle, [{"Group1", 1, [{"Type2", 2}]}]),    

    counter_map(?SUNE, Handle, [{"Group1", 1, [{"Type3", 3}]}]),    

    counter_map(?SUNE, Handle, [{"Group1", 1, [{"Type2", 4}]}]),    

    counter_map(?SUNE, Handle, [{"Group1", 1, [{"Type4", 1}]}]),    
...
cmr(App, Handle) ->
    ...
    %% send counter_map
    %%-------------------------------------------------------------
    counter_map(App, Handle, [{"Group1", 1, [{"Type1", 1}]}]),    
    [{_, A1}] = get_aliases(),
    true = lists:member({1, <<"Group1">>}, A1),
    true = lists:member({{1, 1},  <<"Type1">>}, A1),

    %%-------------------------------------------------------------
    %% reuse the same group alias 
    %%-------------------------------------------------------------
    counter_map(App, Handle, [{"Group2", 1, [{"Type2", 34}]}]),    
    [{_, A2}] = get_aliases(),
    true = lists:member({1, <<"Group2">>}, A2),
    true = lists:member({{1, 34},  <<"Type2">>}, A2),

    %%-------------------------------------------------------------
    %% reuse the same mt alias 
    %%-------------------------------------------------------------
    counter_map(App, Handle, [{"Group2", 1, [{"Type3", 34}]}]),    
    [{_, A3}] = get_aliases(),
    true = lists:member({1, <<"Group2">>}, A3),
    true = lists:member({{1, 34},  <<"Type3">>}, A3),

    %%-------------------------------------------------------------
    %% redefine the mt alias 
    %%-------------------------------------------------------------
    counter_map(App, Handle, [{"Group2", 1, [{"Type3", 99}]}]),    
    [{_, A4}] = get_aliases(),
    true = lists:member({1, <<"Group2">>}, A4),
    true = lists:member({{1, 99},  <<"Type3">>}, A4),
    
    %%-------------------------------------------------------------
    %% redefine the group alias 
    %%-------------------------------------------------------------
    counter_map(App, Handle, [{"Group2", 44, [{"Type3", 99}]}]),    
    [{_, A5}] = get_aliases(),
    true = lists:member({44, <<"Group2">>}, A5),
    true = lists:member({{44, 99},  <<"Type3">>}, A5),
    
    %%-------------------------------------------------------------
    %% redefine the group alias and mt alias
    %%-------------------------------------------------------------
    counter_map(App, Handle, [{"Group1", 44, [{"Type1", 99}]}]),
    [{_, A6}] = get_aliases(),
    true = lists:member({44, <<"Group1">>}, A6),
    true = lists:member({{44, 99},  <<"Type1">>}, A6).
...
%%========================================================================
%% same_app_sequnce_jobs(Config) -> ok.
%% 
%% @doc 
%% 
%% @end
%%========================================================================
same_app_sequnce_jobs(_Config) ->
    %%-------------------------------------------------------------
    %% create app and jobs
    %%-------------------------------------------------------------
    Tables = tables(),
...
add_mr_failed(_Config) ->
    ...
    %%-------------------------------------------------------------
    {ok,    _} = open_trans(),
    {error, _} = create_mr(Job, Job ++ "_new_mr", "Group2", "Type3"),
...
three_apps_same_jobs(_Config) ->
    ...
    {ok, Handle1} = create_app(App1),
    {ok, Handle2} = create_app(App2),
    {ok, Handle3} = create_app(App3),
...
two_apps_diff_jobs(_Config) ->
    ...
    {ok, Handle1} = create_app(App1, ?CM_GRP_1),
    {ok, Handle2} = create_app(App2, ?CM_GRP_2),
...
two_jobs_separate_trans(_Config) ->
    ...
    {ok, Handle1} = create_app(App1, ?CM_GRP_DEF),
...
three_jobs_separate_trans(_Config) ->
    ...
    ok = create_job(Job1, ?JOB_GROUP_1),
    {ok, _} = open_trans(),
    ok = create_job_mr(Job2, [{Job2 ++ "_mr", "Group2", "Type2"}]),
    ok = close_trans(),
    {ok, _} = open_trans(),
    ok = create_job_mr(Job3, [{Job3 ++ "_mr", "Group2", "Type3"}]),
    ok = close_trans(),
