...
%%% #0.    BASIC INFORMATION
%%% ----------------------------------------------------------
%%% %CCaseFile:	pms2_rop_rm_add_mr_1_SUITE.erl %
...
-module(pms2_rop_rm_add_mr_1_SUITE).
...
rm_add_mr_after_cp(_Config) ->
    ...
    %%-------------------------------------------------------------
    %% Remove and add MR to the job not in cp
    %%-------------------------------------------------------------
    %%FF_TRUE = cp is finished
	ok = wait_report_rop(Values, ?FF_TRUE, AppData),
    ...
    %%-------------------------------------------------------------
    %% Check rops after rm and add in single cp
    %%-------------------------------------------------------------    
    ok = wait_report_rop(Values, ?FF_TRUE, AppData),
