...
%%% #0.    BASIC INFORMATION
%%% ----------------------------------------------------------
%%% %CCaseFile:	pms2_rop_rm_add_mr_4_SUITE.erl %
...
-module(pms2_rop_rm_add_mr_4_SUITE).
...
%%========================================================================
%% add_rm_mr_after_cp(Config) -> ok.
%% 
%% @doc 
%% Creates one job with one mr 
%% adds and removes mr after cp
%% @end
%%========================================================================
add_rm_mr_after_cp(_Config) ->
    ...
    %%-------------------------------------------------------------
    %% Add and remove MR in the job not in cp
    %%-------------------------------------------------------------
    %%FF_TRUE ensures that cp is finished
	ok = wait_report_rop(Values, ?FF_TRUE, AppData),
    ok = check_rop_file(2, [?Group2, ?Type2], [?Type3]),
    ok = create_job(Job, get_mrs(MTs2), Attrs),
