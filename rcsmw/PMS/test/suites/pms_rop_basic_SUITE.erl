...
%%% #0.    BASIC INFORMATION
%%% ----------------------------------------------------------
%%% %CCaseFile:	pms_rop_basic_SUITE.erl %
...
-export([tc_1job_1app/1,
	 tc_1job_0app_no_rop/1,
	 tc_1job_2app/1,
	 tc_1job_2app_2groups/1,
	 tc_1job_1app_2inst_sum/1,
	 tc_1job_1app_2inst_avg/1,
	 tc_1job_1app_2inst_min/1,
	 tc_1job_1app_2inst_max/1,
	 tc_1job_1app_2inst_lu/1,
	 tc_1job_1app_2inst_all/1,
	 tc_1job_1app_2inst_all_group/1,
	 tc_1job_1app_2inst_all_multi/1,
	 tc_1job_1app_sum_susp_meas/1,
	 ...
	 tc_1job_1app_2inst_susp_meas_and_val/1,
	 tc_1job_1app_2inst_sum_inc_multi/1,
	 tc_1job_1app_2inst_lu_inc_multi/1
	]).
...
all() -> 
    [
     tc_1job_1app,
     tc_1job_0app_no_rop,
     %% tc_1job_1app_first_rop_susp,
     tc_1job_2app,
     tc_1job_2app_2groups, 
     tc_1job_1app_2inst_sum,
     tc_1job_1app_2inst_avg,
     tc_1job_1app_2inst_min,
     tc_1job_1app_2inst_max,
     tc_1job_1app_2inst_lu,
     tc_1job_1app_2inst_all,
     tc_1job_1app_2inst_all_group,
     tc_1job_1app_2inst_all_multi,
     ...
     tc_1job_1app_2inst_susp_meas_and_val,
     tc_1job_1app_2inst_sum_inc_multi
     %% tc_1job_1app_2inst_lu_inc_multi %temporarily removed
    ].
...
%% @doc
%% Start a PM job with one counter, but no application, and verify that 
%% no ROP file is created.
%% @end
tc_1job_0app_no_rop(_Config) -> 
    {ok, _} = open_trans(),
    ok = create_job_mr(?JOB1, {?MR1_1, ?ROPGroup1, ?ROPType1Sum}),
    ok = close_trans(),
    try
	WT = get_rop_wait_time(?REP_PERIOD1, 1),
	{error, timeout} = wait_rop_file(1, WT)
    catch
	_:Error ->
	    ?CLEAN_UP_AND_FAIL(Error, [?JOB1], [])
    end,
    clean_up([?JOB1], []).
...
tc_1job_2app(_Config) -> 
    {ok, _} = pms_erl_app:start(?NACKA, ?LDN1, 
		      ?MEAS_VAL_AVG_2([1])),
    {ok, _} = pms_erl_app:start(?RONNIE, ?LDN2, 
		      ?MEAS_VAL_AVG_2([1])),
    initialize(?NACKA, [?ROPGroup2]), 
    initialize(?RONNIE, [?ROPGroup2]), 
    {ok, _} = open_trans(),
    ok = create_job_mr(?JOB1, {?MR1_1, ?ROPGroup2, ?ROPType7Avg}),
    ok = close_trans(),
    try
	Data = get_first_rop_file(?REP_PERIOD1),
	nomatch = re:run(Data, "suspect")
...
%% aggregation SUM and verify that a ROP file is generated and that the counter
%% value is aggregated.
%% @end
tc_1job_1app_2inst_sum(_Config) -> 
    {ok, _} = pms_erl_app:start(?NACKA, ?LDN1, 
		      ?MEAS_VAL_SUM([1])),
    ...
    try
	Data = get_first_rop_file(?REP_PERIOD1),
	ok = check_rop_cont(Data, ["<r p=\"1\">2</r>"])
    catch
	_:Error ->
	    ?CLEAN_UP_AND_FAIL(Error, [?JOB1], [?NACKA, ?RONNIE])
    end,
...
%% @doc
%% Start two instances of an application and a PM job with one counter with
%% aggregation AVG and verify that a ROP file is generated and that the counter
%% value is aggregated.
%% @end
...
tc_1job_1app_2inst_min(_Config) -> 
    ...
    initialize(?RONNIE, [?ROPGroup1]), 
    {ok, _} = open_trans(),
    ok = create_job_mr(?JOB1, {?MR1_1, ?ROPGroup1, ?ROPType3Min}),
    ok = close_trans(),
    try
	Data = get_first_rop_file(?REP_PERIOD1),
...
tc_1job_1app_2inst_max(_Config) -> 
    ...
    initialize(?RONNIE, [?ROPGroup1]), 
    {ok, _} = open_trans(),
    ok = create_job_mr(?JOB1, {?MR1_1, ?ROPGroup1, ?ROPType4Max}),
    ok = close_trans(),
    try
	Data = get_first_rop_file(?REP_PERIOD1),
	ok = check_rop_cont(Data, ["<r p=\"1\">3</r>"])
    catch
	_:Error ->
	    ?CLEAN_UP_AND_FAIL(Error, [?JOB1], [?NACKA, ?RONNIE])
    end,
...
tc_1job_1app_2inst_lu(_Config) -> 
    ...
    try
	Data = get_first_rop_file(?REP_PERIOD1),
	ok = check_rop_cont(Data, ["<r p=\"1\">5</r>"])
    catch
	_:Error ->
	    ?CLEAN_UP_AND_FAIL(Error, [?JOB1], [?NACKA, ?RONNIE])
    end,
...
%% @doc
%% Start two instances of an application and a PM job with five counters with
%% different aggregation and verify that a ROP file is generated and that the 
%% counter values are aggregated.
%% @end
tc_1job_1app_2inst_all(_Config) -> 
    {ok, _} = pms_erl_app:start(?NACKA, ?LDN1, 
		      ?MEAS_VALS_ALL([1],[2],[3],[4],[5])),
    {ok, _} = pms_erl_app:start(?RONNIE, ?LDN1,
		      ?MEAS_VALS_ALL([1],[4],[4],[3],[5])),
    initialize(?NACKA, [?ROPGroup1]), 
    initialize(?RONNIE, [?ROPGroup1]), 
    {ok, _} = open_trans(),
    ok = create_job_mr(?JOB1, [{?MR1_1, ?ROPGroup1, ?ROPType1Sum},
			       ...
			       {?MR1_4, ?ROPGroup1, ?ROPType4Max},
			       {?MR1_5, ?ROPGroup1, ?ROPType5LU}]),
    ok = close_trans(),
    try
	Data = get_first_rop_file(?REP_PERIOD1),
	ok = check_rop_cont(Data, ["<r p=\"1\">2</r>",
...
%% counter values are aggregated.
%% @end
tc_1job_1app_2inst_all_group(_Config) -> 
    {ok, _} = pms_erl_app:start(?NACKA, ?LDN1, 
		      ?MEAS_VALS_ALL([1],[2],[3],[4],[5])),
    {ok, _} = pms_erl_app:start(?RONNIE, ?LDN1,
		      ?MEAS_VALS_ALL([1],[4],[4],[3],[5])),
...
%% @doc
%% Start two instances of an application and a PM job with five multi value 
%% counters with different aggregation and verify that a ROP file is generated 
%% and that the counter values are aggregated.
%% @end
tc_1job_1app_2inst_all_multi(_Config) -> 
    {ok, _} = pms_erl_app:start(?NACKA, ?LDN1, 
		      ?MEAS_VALS_ALL_MULT([1,1],
					  [1,2,3],
					  [1,3,6,9],
					  [3,4,5,6,7],
					  [4,5,6,7,8,9])),
    {ok, _} = pms_erl_app:start(?RONNIE, ?LDN1,
		      ?MEAS_VALS_ALL_MULT([1,2],
					  [3,4,5],
					  [3,4,5,9],
					  [1,3,6,5,7],
					  [4,5,6,7,8,9])),
    ...
    ok = create_job_mr(?JOB1, [{?MR1_1, ?ROPGroupMult, ?ROPType11SumMult},
			       {?MR1_2, ?ROPGroupMult, ?ROPType12AvgMult},
			       {?MR1_3, ?ROPGroupMult, ?ROPType13MinMult},
			       {?MR1_4, ?ROPGroupMult, ?ROPType14MaxMult},
			       {?MR1_5, ?ROPGroupMult, ?ROPType15LUMult}]),
    ok = close_trans(),
    try
	Data = get_first_rop_file(?REP_PERIOD1),
	ok = check_rop_cont(Data, ["<r p=\"1\">2,3</r>",
				   "<r p=\"2\">2,3,4</r>", 
				   "<r p=\"3\">1,3,5,9</r>", 
				   "<r p=\"4\">3,4,6,6,7</r>", 
				   "<r p=\"5\">4,5,6,7,8,9</r>"])
...
%% @doc
%% Start two instances of an application and a PM job with one counter with
%% aggregation SUM and verify that the value is blanked out and suspect 
%% marked when one instance reports -2 and the other -1.
%% @end
tc_1job_1app_2inst_susp_meas_and_val(_Config) ->
    {ok, _} = pms_erl_app:start(?NACKA, ?LDN1, 
		      ?MEAS_VAL_SUM([-2])),
    {ok, _} = pms_erl_app:start(?RONNIE, ?LDN1, 
		      ?MEAS_VAL_SUM([-1])),
...
%% marked when the lengths are inconsistent.
%% @end
tc_1job_1app_2inst_sum_inc_multi(_Config) -> 
    {ok, _} = pms_erl_app:start(?NACKA, ?LDN1, 
		      ?MEAS_VAL_SUM_MULT([1,2])),
    {ok, _} = pms_erl_app:start(?RONNIE, ?LDN1, 
...
%% @doc
%% Start two instances of an application and a PM job with one multivalue 
%% counter with aggregation LU and verify that the measurement is not suspect
%% marked as long as the length of the last reported is not inconsistent.
%% @end
tc_1job_1app_2inst_lu_inc_multi(_Config) -> 
    {ok, _} = pms_erl_app:start(?NACKA, ?LDN1, 
				?MEAS_VAL_LU_MULT([1,2,3,4,5])),
    initialize(?NACKA, [?ROPGroupMult]), 
    %% {ok, _} = pms_erl_app:start(?RONNIE, ?LDN1, 
    %% 				?MEAS_VAL_LU_MULT([1,2,3,4,5])),
    AppFun = 
	fun() ->
		{ok, APid} = 
		    pms_erl_app:start(?RONNIE, ?LDN1, 
				      ?MEAS_VAL_LU_MULT([1,2,3,4,5,6]), true),
				      %% [{ask, true}]),
		initialize(?RONNIE, [?ROPGroupMult]), 
		%% wait_subscribe([{?ROPGroupMult, [?ROPType15LUMult]}]),
		wait_subscribe([?ROPGroupMult]),
		wait_report(wait, {1000, [{?ROPGroupMult,
					   ?MEAS_VAL_LU_MULT([1,2,3,4,5,6])}]}),
		wait_report(wait, {1000, [{?ROPGroupMult,
					   ?MEAS_VAL_LU_MULT([1,2,3,4,5,6])}]}),
		APid ! {ask,undefined}
		%% wait_subscribe([])
    ...
    WPid = spawn(AppFun),
    {ok, _} = open_trans(),
    ok = create_job_mr(?JOB1, {?MR1_1, ?ROPGroupMult, ?ROPType15LUMult}),
    ok = close_trans(),
    try
	Data = get_first_rop_file(?REP_PERIOD1),
	ok = check_rop_cont(Data, [])
    catch
	_:Error ->
	    exit(WPid, kill),
	    ?CLEAN_UP_AND_FAIL(Error, [?JOB1], [?NACKA, ?RONNIE])
