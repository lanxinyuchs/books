...
%%% #0.    BASIC INFORMATION
%%% ----------------------------------------------------------
%%% %CCaseFile:	pms_rop_ext_1_SUITE.erl %
%%% @version /main/R3A/R4A/R6A/R11A/1
...
%% start one job with one MT
%% wait for ROP
%% start 2nd job with same group but anoter MT
%% next ROP should only have job 1 data
%% next ROP should have both job's data
%% @end
tc_1_plus_1_job(_Config) ->
    {ok, _} = open_trans(),
    ok = create_job_mr(?JOB1, [{?MR1_1, ?ROPGroup1, ?ROPType1Sum}]),
    ok = close_trans(),
    {ok, _App} = pms_erl_app:start(?NACKA, 
				   [{?LDN1, 
				     [{?RP_10_SEC, 
				       [{?ROPGroup1, [?ROPType1Sum, ?ROPType2Avg]}]}]}
				   ]),
    
    initialize(?NACKA, [?ROPGroup1]), 

    try
	Data1 = get_rop_file_n(1, ?REP_PERIOD1),
	ok = check_rop_cont(Data1, [?JOB1], [?JOB2])
    catch
	_:Error1 ->
	    ?CLEAN_UP_AND_FAIL(Error1, [?JOB1, ?JOB2], [?NACKA], ?REP_PERIOD1)
    end,
    ...
    try
	Data2 = get_rop_file_n(2, ?REP_PERIOD1),
	ok = check_rop_cont(Data2, [?JOB1], [?JOB2])
    catch
	_:Error2 ->
	    ?CLEAN_UP_AND_FAIL(Error2, [?JOB1, ?JOB2], [?NACKA], ?REP_PERIOD1)
    end,

    try
	Data3 = get_rop_file_n(3, ?REP_PERIOD1),
	ok = check_rop_cont(Data3, [?JOB1, ?JOB2])
    catch
	_:Error3 ->
	    ?CLEAN_UP_AND_FAIL(Error3, [?JOB1, ?JOB2], [?NACKA], ?REP_PERIOD1)
    end,



    clean_up([?JOB1, ?JOB2], [?NACKA], ?REP_PERIOD1, 1000).
