...
%%% #0.    BASIC INFORMATION
%%% ----------------------------------------------------------
%%% %CCaseFile:	pms_rop_rm_add_mr_1_SUITE.erl %
...
-module(pms_rop_rm_add_mr_1_SUITE).
...
%%========================================================================
%% rm_add_mr_after_cp(Config) -> ok.
%%
%% @doc
%% Creates one job with two mrs
%% removes and adds mr after cp
%% @end
%%========================================================================
rm_add_mr_after_cp(_Config) ->
    %%-------------------------------------------------------------
    %% Definitions
    %%-------------------------------------------------------------
    App      = ?SUNE,
    ...
    %%-------------------------------------------------------------
    %% Remove and then immediately add MT to the PmJob,
    %% while outside of CP.
    %%-------------------------------------------------------------
    ...
    %%-------------------------------------------------------------
    %% Check second ROP file, generated immediately after removing
    %% and adding a MT in single CP (without waiting for full RP
    %% to finish in between). Second ROP file should not contain
    %% the removed MT.
    %%-------------------------------------------------------------
    ok = wait_pmi_report_rop(Values, ?FF_TRUE, AppData),
    ...
    %%-------------------------------------------------------------
    %% After a full RP has passed since the last change, ROP
    %% contents should be back to their original state.
    %%-------------------------------------------------------------
    ok = wait_pmi_report_rop(Values, ?FF_TRUE, AppData),

    %% Third ROP file should contain ROPType2Avg.
    ok = check_rop_file(3, [?ROPGroup1, ?ROPType1Sum, ?ROPType2Avg]),
