...
%%% #0.    BASIC INFORMATION
%%% ----------------------------------------------------------
%%% %CCaseFile:	pms_rop_rm_add_mr_2_SUITE.erl %
...
-module(pms_rop_rm_add_mr_2_SUITE).
...
%% 
%% @doc 
%% Create one PmJob with two MRs.
%% Remove MR in CP and add it after CP.
%% @end
%%========================================================================
rm_mr_in_cp_add_after(_Config) ->
    ...
    GP       = ?GP_1_MIN,
    Job      = "rm_add_mr_after_cp",

    MTs1     = [{?ROPGroup1, [?ROPType1Sum, ?ROPType2Avg]}],
    ...
    Values  = get_pmi_values(MTs1, LDN, [1, 2]),
    ...
    %%-------------------------------------------------------------
    %% First (incomplete) RP should generate no ROP files.
    %%-------------------------------------------------------------
    ok = wait_pmi_report_rop(Values, ?FF_TRUE, AppData),
    ...
    %%-------------------------------------------------------------
    ok = wait_pmi_report_rop(Values, ?FF_TRUE, AppData),
    ok = check_rop_file(1, [?ROPGroup1, ?ROPType1Sum, ?ROPType2Avg]),

    %%-------------------------------------------------------------
    %% Remove MT during the CP.
    %%-------------------------------------------------------------
    {ok, ReportId} = enter_cp_pmi(GP),

    ok = delete_mr(Job, ?ROPType2Avg),
    ok = wait_until_pmi_subscribe({GP, MTs2}),

    ok = exit_cp_pmi(Values, AppData, ReportId),

    %%-------------------------------------------------------------
    %% Re-add the MT after exiting the CP (also, removing
    %% the MT should not be immediately apparent).
    %%-------------------------------------------------------------
    ok = check_rop_file(2, [?ROPGroup1, ?ROPType1Sum, ?ROPType2Avg]),

    ok = create_job(Job, get_mrs(MTs1), JobAttrs),
    ok = wait_until_pmi_subscribe({GP, MTs1}),

    %%-------------------------------------------------------------
    %% After a full RP has passed since the last change, ROP file
    %% should now reflect the deleted MT.
    %%------------------------------------------------------------- 
    ok = wait_pmi_report_rop(Values, ?FF_TRUE, AppData),
    ...
    %%-------------------------------------------------------------
    %% And after another RP, the ROPs contents should now contain
    %% data for both MTs.
    %%------------------------------------------------------------- 
    ok = wait_pmi_report_rop(Values, ?FF_TRUE, AppData),
    ok = check_rop_file(4, [?ROPGroup1, ?ROPType1Sum, ?ROPType2Avg]),
...
%%========================================================================
%% enter_cp(GP) -> {ok, ReportId} | {error, Description}
%%
%% Enters collection period.
%%========================================================================
enter_cp_pmi(GP) ->
    receive
	{copy, _, {pmiReport, {_, ReportId, _}}} ->
	    {ok, ReportId};
	{copy, _, Unexpected} ->
	    ct:pal("Enter CP: Unexpected ~p~n", [Unexpected]),
	    {error, Unexpected}
    after (GP + 5) * 1000 ->
...
%%========================================================================
%% exit_cp(VFFs, AppData, ReportId) -> ok
%%
%% Ends collection period.
%%========================================================================
exit_cp_pmi(LDNValues, {App, Handle, GP}, ReportId) ->
    [rop_data_pmi(App, Handle, GP, ReportId, LDN, Values) ||
    {LDN, Values} <- append_empty_fragment(LDNValues)],
    ok.


%%========================================================================
%% wait_pmi_report_rop(LDNValues, FF, AppData) -> ok | {error, cause}
%%
%% Waits for PMI ROP report (modifies test_lib interface
%% so that the function can be used the same as the PMI2
%% version).
%%========================================================================
wait_pmi_report_rop(RawValues, FF, AppData) ->
    Values = case FF of
        ?FF_TRUE -> append_empty_fragment(RawValues);
               _ -> RawValues
    end,

    ?LIB:wait_pmi_report_rop(Values, AppData).


%%========================================================================
%% append_empty_fragment(LDNValues) -> ValuesWithEmptyFragment
%%
%% Appends an empty fragment to the given LDNValues
%% (used instead of FinalFlag in PMI2).
%%========================================================================
append_empty_fragment(LDNValues) ->
    [{LDN, _} | _] = LDNValues,
    EmptyFragment = [{LDN, []}],
    LDNValues ++ EmptyFragment.
...
rop_data_pmi(A, B, C, D, E, F) -> ?LIB:rop_data_pmi(A, B, C, D, E, F).
