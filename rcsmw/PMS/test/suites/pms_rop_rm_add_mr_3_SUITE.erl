...
%%% #0.    BASIC INFORMATION
%%% ----------------------------------------------------------
%%% %CCaseFile:	pms_rop_rm_add_mr_3_SUITE.erl %
...
-module(pms_rop_rm_add_mr_3_SUITE).
...
%%% Rev        Date       Name        What
%%% -----      ---------  --------    ------------------------
%%% R5L/1      2016-04-04 edamkon     Copied from pms_rop_rm_add_mr_2_SUITE
%%% ----------------------------------------------------------
%%%
%%% ----------------------------------------------------------
%%% #2.    EXPORT LISTS
...
%%========================================================================
%% rm_add_mr_in_cp(Config) -> ok.
%% 
%% @doc 
%% Create one job with two MRs 
%% Remove and add an MR in CP
%% @end
%%========================================================================
rm_add_mr_in_cp(_Config) ->
    %%-------------------------------------------------------------
    %% Definitions
    %%-------------------------------------------------------------
    App      = ?SUNE,
    ...
    %%-------------------------------------------------------------
    ok = create_job(Job, MR1, JobAttrs),
    ...
    %%-------------------------------------------------------------
    ok = wait_pmi_report_rop(Values, ?FF_TRUE, AppData),
    ok = check_rop_file(1, [?ROPType1Sum, ?ROPType2Avg]),

    %%-------------------------------------------------------------
    %% Remove and add MT back to the job during the CP.
    %%-------------------------------------------------------------
    ...
    %%-------------------------------------------------------------
    %% ROP files should immediately reflect the removal of the MR.
    %%-------------------------------------------------------------
    ok = wait_pmi_report_rop(Values, ?FF_TRUE, AppData),
    ok = check_rop_file(3, [?ROPType1Sum], [?ROPType2Avg]),

    %%-------------------------------------------------------------
    %% But only after a full RP should the re-adding of the MR
    %% be visible.
    %%-------------------------------------------------------------
    ok = wait_pmi_report_rop(Values, ?FF_TRUE, AppData),
    ok = check_rop_file(4, [?ROPType1Sum, ?ROPType2Avg]),
