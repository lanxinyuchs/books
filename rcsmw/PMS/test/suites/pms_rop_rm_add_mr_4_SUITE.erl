...
%%% #0.    BASIC INFORMATION
%%% ----------------------------------------------------------
%%% %CCaseFile:	pms_rop_rm_add_mr_4_SUITE.erl %
...
-module(pms_rop_rm_add_mr_4_SUITE).
...
%% Add and remove MT after CP
%% @end
%%========================================================================
add_rm_mr_after_cp(_Config) ->
    %%-------------------------------------------------------------
    %% Definitions
    %%-------------------------------------------------------------
    App      = ?SUNE,
    ...
    Values   = get_pmi_values(MTs2, LDN, [[1], [2]]),
    ...
    %%-------------------------------------------------------------
    %% Check first ROP, should contain only first MR.
    %%-------------------------------------------------------------
    ok = wait_pmi_report_rop(Values, ?FF_TRUE, AppData),
    ok = check_rop_file(1, [?ROPGroup1, ?ROPType1Sum]),

    %%-------------------------------------------------------------
    %% Add and immediately remove MT2 from the job, after the CP.
    %% (we are in period immediately after CP after having waited
    %% for the ROP file).
    %%-------------------------------------------------------------
    ok = create_job(Job, MR2, JobAttrs),
    ...
    %%-------------------------------------------------------------
    %% Recheck after an additional RP, the 3rd ROP file shouldn't
    %% contain MT2 either, since it was immediately removed.
    %%-------------------------------------------------------------
    ok = wait_pmi_report_rop(Values, ?FF_TRUE, AppData),
