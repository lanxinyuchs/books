...
%%% #0.    BASIC INFORMATION
%%% ----------------------------------------------------------
%%% %CCaseFile:	pms_rop_rm_add_mr_5_SUITE.erl %
...
-module(pms_rop_rm_add_mr_5_SUITE).
...
-export([add_mr_in_cp_rm_after/1]).
...
%%========================================================================
%% add_rm_mr_in_cp(Config) -> ok.
%% 
%% @doc 
%% Creates one job with one mr 
%% adds and removes mr during cp
%% @end
%%========================================================================
add_mr_in_cp_rm_after(_Config) ->
    %%-------------------------------------------------------------
    %% Definitions
    %%-------------------------------------------------------------
    App     = ?SUNE,
    GP      = ?GP_1_MIN,
    Job     = "add_mr_in_cp_rm_after",
    ...
    %%-------------------------------------------------------------
    Values = get_pmi_values(MTs1, LDN, [[1221]]),
    ok = wait_pmi_report_rop(Values ++ [{LDN, []}], AppData),
    ok = wait_pmi_report_rop(Values ++ [{LDN, []}], AppData),
    ok = check_rop_file(1, [?ROPGroup1, ?ROPType1Sum], [?ROPType3Min]),

    %%-------------------------------------------------------------
    %% Add MR during cp, rm after cp
    %%-------------------------------------------------------------
    %%FF_FALSE ensures that cp is not finished
    {ok, Id} = enter_cp_pmi(GP),
    ok = create_job(Job, get_mrs(MTs2), JobAttrs),
    ok = wait_until_pmi_subscribe({GP, MTs2}),
    exit_cp_pmi(Values, AppData, Id),
