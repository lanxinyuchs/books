...
LOG = logging.getLogger('ACTIVATE_NFS')
...
def main(job_id, operation_data, upgrade_job, lcm_config, callback_when_finished):
    ...
    if (upgrade_job.get_state() == lcm_upgrade_job.get_state_enum().PREPARE_COMPLETED):
        ...
        LOG.info("Activate nfs initiated for JOB_ID: %s" % (job_id))
        return activate
    else:
...
class EnumWorkingState:
    ...
    WAIT_FOR_EXPORT_BACKUP_RESPONSE = "WAIT_FOR_EXPORT_BACKUP_RESPONSE"
...
class Activate(lcm_wf_utility.GenAsyncQueue):
    ...
    def main(self, operation_data):
        ...
        LOG.debug('main(self, *operation_data) - operation_data: %s'
                  % (operation_data))
        LOG.info("LCM Workflow Activate NFS Version: %s, LCM Framework Version: %s (job id: %s)"
            ...
            if (self.upgrade_job.resource.operation != None and
                ...
                try:
                    ...
                    if self.upgrade_job.resource.fallback_timeout != None:
                        ...
                        LOG.info("Activate Fallback timer started (%s) at restore. Timeout:  %s" %
    ...
    def handle_main(self, operation_data):
        ...
        try:
            ...
            self.fallback_timer = threading.Timer(self.get_fallback_timeout(self.ext_operation_data),
                                                  self.timeout)
            self.fallback_timer.start()
            LOG.info("Activate Fallback timer started (%s). Timeout:  %s" %
            ...
            self.upgrade_job.resource.wf_name = lcm_upgrade_job.get_workflow_enum().ACTIVATE_NFS_OPERATION
            ...
            self.action_id = self.vnf_comm_handle_from.create_backup(self.backup_name)
    ...
    def handle_confirm(self, operation_data, continues = False, add_info = list()):
        ...
        try:
            ...
            self.__remove_backup_startup_path()
    ...
    def handle_backup_status(self, stat_operation_data):
        ...
        try:
            ...
            elif self.working_state == EnumWorkingState.WAIT_FOR_EXPORT_BACKUP_RESPONSE:
                self.__handle_export_backup_response(stat_operation_data)
            else:
        ...
        try:
            if self.__is_vnf_known(upg_operation_data['vnf_id']):
                if upg_operation_data['status']['status_code'] == "WAITING_FOR_COMMIT":
                    ...
                    LOG.error("__handle_upgrade_status(self, upg_operation_data) - "
    ...
    def __continue_activation(self, upg_operation_data):
        ...
        if (upg_operation_data['vnf_id'] == self.upgrade_job.get_to_vnf_id()):
            ...
            self.vnf_comm_handle_from.stop_traffic()
            self.upgrade_job.resource.is_traffic_stoped = True
            self.__set_resource()

            self.__copy_esi()

            LOG.info("Requesting From-VNF to be stoped")
            self.__set_progress(
                62,
            ...
            self.__get_vnf_comm_to().start_traffic()
            self.__set_progress(
                65,
    ...
    def __handle_create_backup_response(self, operation_data):
        ...
        self.__set_progress(
            20,
            self.progress_wf.get_enum(self.progress_wf.EXPORTING_BACKUP_FOR_FROM_STATE_VNF))
        self.__prepare_vnf_backup_path()
        backup_path_uri = 'file:///shared/' + self.lcm_config.get_external_backup_path()
        LOG.info("Requesting From-VNF (%s) to export a backup of configuration to URI '%s'"
                 % (self.upgrade_job.get_vnf_id(), backup_path_uri))
        self.working_state = EnumWorkingState.WAIT_FOR_EXPORT_BACKUP_RESPONSE
        self.action_id = self.vnf_comm_handle_from.export_backup(backup_path_uri, self.backup_name)

        # Wait for backup_status
        LOG.debug("Waiting for created To-VNF to be up and running")
        return True

    def __prepare_vnf_backup_path(self):
        """ Ensures that the path exists and file name do not exist """
        LOG.debug("__prepare_vnf_backup_path(self)")

        self.vnf_backup_dir_path = "%s/%s" % (
            self.__get_vnf_global_disk_path(self.upgrade_job.get_vnf_id()),
            self.lcm_config.get_external_backup_path())

        LOG.debug("__prepare_vnf_backup_path(...) - vnf_backup_path on global disk: %s"
                  % (self.vnf_backup_dir_path))
        if not os.path.isdir(self.vnf_backup_dir_path):
            LOG.debug("__prepare_vnf_backup_path(...) - Creating the VNF backup path: %s"
                      % (self.vnf_backup_dir_path))
            os.makedirs(self.vnf_backup_dir_path)

    def __remove_backup_startup_path(self):
        """ Removes the external backup file from startup directory """
        LOG.debug("__remove_backup_startup_path(self)")

        self.vnf_startup_backup_dir_path = "%s/%s" % (
            self.__get_vnf_global_disk_path(self.upgrade_job.get_to_vnf_id()),
            self.lcm_config.get_vnf_backup_path())
        startup_backup_file_path = "%s%s" % (self.vnf_startup_backup_dir_path,
                                             Activate.BACKUP_FILE_NAME_AT_STARTUP)
        if os.path.exists(startup_backup_file_path):
            try:
                os.remove(startup_backup_file_path)
                LOG.info("The backup file used at startup during activation "
                         "has been removed (%s)" % (startup_backup_file_path))
            except OSError as exception:
                LOG.info("Failed to remove the backup file from the VNF "
                         "startup file path (%s). Reason: %s"
                         % (startup_backup_file_path, exception))
        else:
            LOG.info("The backup file used at startup do not exist when it "
                     "should (ignored), path is %s" % (startup_backup_file_path))

    def __get_vnf_global_disk_path(self, vnf_id):
        """
        Returns root path to VNF global disk
        This should preferable be retreivable from VNFM as a common function.
        Will look into this after current push has been submited (is. so that Jaguar
        team can start verification
        """
        LOG.debug("__get_vnf_global_disk_path(self, vnf_id) - vnf_id: %s" % (vnf_id))

        vnf_global_disk_path = "%s%s" % (self.lcm_config.get_global_disk_path(), vnf_id)

        if not os.path.isdir(vnf_global_disk_path):
            LOG.error("__get_vnf_global_disk_path(self, vnf_id) - The global root disk path "
                      "for VNF didn't exist. Can't continue. vnf_global_disk_path: %s"
                      % (vnf_global_disk_path))
            raise lcm_wf_utility.WorkflowException(
                "The global root disk path for VNF didn't "
                "exist. Can't continue. vnf_global_disk_path: %s"
                % (vnf_global_disk_path))

        LOG.debug("__get_vnf_global_disk_path(...) - RETURNS %s" % (vnf_global_disk_path))
        return vnf_global_disk_path

    def __handle_export_backup_response(self, operation_data):
        """
        Handling of activation after response has been received from
        export backup
        """
        LOG.debug("__handle_export_backup_response(self, operation_data)"
                  " - operation_data: %s" % (operation_data))

        self.working_state = EnumWorkingState.IDLE
        self.__check_backup_response(operation_data, 'export')


        self.__set_progress(
            40,
            self.progress_wf.get_enum(self.progress_wf.CREATING_TO_STATE_VNF))
        from_vnf_info = self.vnfm_comm_handle.get_vnf_info(
            self.upgrade_job.get_vnf_id(),
            [vnfm_api_comm.VnfmComm.EXT_VIRTUAL_LINKS])
        ext_virtual_links = from_vnf_info.get_ext_virtual_links()
        ...
        self.__instantiate_vnf(self.vnf_backup_dir_path,
                               operation_data['backup_file'],
                               from_vnf_info.instance_name,
                               from_vnf_info.description,
                               ext_virtual_links)
        self.__is_cancel_requested()
        self.__is_fallback_timeout()
        self.__subscribe_for_lcop(self.upgrade_job.to_vnf_id, ['heal', 'operate', 'terminate'])
        LOG.info("Waiting for created To-VNF to be up and running")
        self.__set_progress(
            50,
    ...
    def __handle_upgrade_status(self, operation_data):
        """ Checks that response is OK otherwise exception is raised """
        LOG.debug("__handle_upgrade_status(operation_data)"
                  " - operation_data: %s" % (operation_data))

        if operation_data['status']['status_code'] == "INFO":
            LOG.info("Upgrade information received from VNF involved in "
                     "the activation procedure. Message from VNF - '%s'"
                     % (operation_data['status']['info']))

        elif operation_data['status']['status_code'] == "FAILURE":
            LOG.error("__handle_upgrade_status(self, operation_data) - "
                      "Got an upgrade response from VNF reporting that activation"
                      " went wrong. VNF response: %s, upgrade job: %s"
                      % (operation_data, self.upgrade_job))
            raise lcm_wf_utility.WorkflowException(
                "Got an upgrade response from VNF reporting that activation"
                " went wrong. VNF response: %s, upgrade job: %s"
                % (operation_data, self.upgrade_job))
        else:
            LOG.warning("Unknown status code received in an upgrade response "
                        "(ignored) when working state is: %s. Received operation"
                        " data: %s, own upgrade job: %s"
                        % (operation_data, self.upgrade_job, self.working_state))
    ...
    def __handle_interrupt(self, interrupt_type, operation_data):
        ...
        """
        LOG.debug('__handle_interrupt(self, interrupt_type, operation_data) - interrupt_type: %s, '
        ...
        export_result = self.__export_esi(self.__get_vnf_comm_to(),
                                          self.upgrade_job.get_to_vnf_id(),
                                          self.ESI_FILE_NAME_ROLLBACK,
                                          "Requesting To-VNF to export ESI to LCM tmp directory")
        self.__heal_from_vnf_if_needed("The upgrade job (%s) has been interrupted by %s"
                                       % (interrupt_type, self.upgrade_job.get_job_id()),
    ...
    def handle_error(self, reason, operation_data):
        ...
        self.__unsubscribe_for_lcop(self.upgrade_job.resource.subscription_url_id)
        LOG.info("Requesting To-VNF to export ESI to LCM tmp directory")
        export_result = False
        try:
            export_result = self.__export_esi(self.__get_vnf_comm_to(),
                                              self.upgrade_job.get_to_vnf_id(),
            ...
            LOG.info("Failed to export ESI. Reason: %s" % (exception))
        self.__terminate_to_vnf()
    ...
    def __terminate_to_vnf(self):
        ...
        try:
            if self.upgrade_job.get_to_vnf_id() != None:
                LOG.info("Requesting VNFM to terminate To-VNF and delete "
    ...
    def __start_configuration(self, is_configuration_stoped, is_traffic_stoped):
        """ Starts the configuration if needed i.e. removes the configuration lock """
        LOG.debug("__start_configuration(is_configuration_stoped, is_traffic_stoped)"
                  " - is_configuration_stoped: %s, is_traffic_stoped: %s"
                  % (is_configuration_stoped, is_traffic_stoped))
        try:
            if (is_configuration_stoped == True and is_traffic_stoped == False):
                LOG.info("Requesting From-VNF %s to allow configuration changes"
                         % (self.vnf_comm_handle_from))
                self.vnf_comm_handle_from.start_configuration()

        except Exception as exception:
            LOG.error("__start_configuration_vnf(self, is_configuration_stoped, "
                      "is_traffic_stoped) - Exception caught when sending operation"
    ...
    def __react_to_any_cancel_request(self, is_interrupt_allowed):
        ...
        #TODO: remove log req
        LOG.debug("__react_to_any_cancel_request(is_interrupt_allowed)"
                  " - is_interrupt_allowed: %s" % (is_interrupt_allowed))
    ...
    def __is_cancel_requested(self):
        ...
        if not self.is_cancel_ongoing:
            if self.cancel_requested:
                LOG.info("Operation %s is aborted since operation "
                         "Cancel has been received"
                         % (self.upgrade_job.resource.operation))
                self.__set_progress(
                    90,
    ...
    def __instantiate_vnf(self, vnf_backup_dir_path, backup_file_name, instance_name,
                          description, ext_virtual_links):
        """ Instantiates a VNF """
        LOG.debug("__instantiate_vnf(self, vnf_backup_dir_path, backup_file_name, "
                  "instance_name, description, ext_virtual_links)"
                  " - vnf_backup_dir_path: %s, backup_file_name: %s, instance_name: %s, "
                  "description: %s, ext_virtual_links: %s"
                  % (vnf_backup_dir_path, backup_file_name, instance_name,
                     description, ext_virtual_links))
        ...
        # Create vnf instance
        http_payload = self.__read_backup_file_to_http_data(vnf_backup_dir_path, backup_file_name)
            ...
            if vnf_id != None:
                ...
                if self.vnfm_comm_handle.is_supported(lcm_util.EnumVersion.VER_3_0):
                    operate_state_rsp_data = self.vnfm_comm_handle.operate_state(
                        vnf_id, state)
                    self.__wait_for_lc_op_ok(
                        operate_state_rsp_data['vnfLcOpId'],
                        'After requested change of operate state of VNF with ID: %s' % (vnf_id),
                        True)
                else:
                    ...
                    LOG.info("Waits for 30 seconds to be sure that the %s-VNF has been operated."
                             ...
                             % (which_vnf, lcm_util.EnumVersion.VER_2_0,
    ...
    def __remove_vnf_id(self, vnf_id, which_vnf):
        ...
        try:
            if vnf_id != None:
                ...
                vnfm_api_comm.VnfmCommException) as exception:
            LOG.info("__remove_vnf_id(self, vnf_id, which_vnf)) - vnf_id: %s. LCM Exception "
            ...
            LOG.warning("__remove_vnf_id(self, vnf_id, which_vnf)) - vnf_id: %s. Exception "
                        "caught when trying to remove given VNF id (ignored)" % (vnf_id))
            LOG.exception(exception)
            self.add_infos.append("WARNING: Removal of %sVNF id '%s' failed. Please, ensure that"
                                  " this VNF id is manually removed to avoid resource leakage"
    ...
    def __wait_for_lc_op_ok(self, vnf_lc_op_id, lcop_request_info_str, is_interrupt_allowed):
        ...
        LOG.debug("__wait_for_lc_op_ok(self, "
                  "vnf_lc_op_id, lcop_request_info_str, is_interrupt_allowed) - vnf_lc_op_id: %s"
                  "lcop_request_info_str: %s, is_interrupt_allowed:%s" % (
                      vnf_lc_op_id, lcop_request_info_str, is_interrupt_allowed))
        while (True):
            ...
            if  vnf_lc_op_rsp['status'] == 'success':
                ...
                LOG.error("__wait_for_lc_op_ok(self, vnf_lc_op_id, "
                          "lcop_request_info_str, is_interrupt_allowed)"
    ...
    def __execute_auto_verify(self, *args, **kwargs):
        ...
        if (verifier.define_result(verify_check_results) == wf_module.VerifyCheckResult.FAILURE):
            LOG.warning("__execute_auto_verify(self, *args, **kwargs)"
            ...
            LOG.info("__execute_auto_verify(self, *args, **kwargs)"
    ...
    def __heal_from_vnf_if_needed(self, reason, is_stop_traffic_requested):
        """ Used to request for heal of From-VNF, if needed """
        LOG.debug("__heal_from_vnf_if_needed(self, reason, is_stop_traffic_requested) -"
                  " reason: %s, is_stop_traffic_requested: %s" %
                  (reason, is_stop_traffic_requested))
    ...
    def __heal_from_vnf(self, reason):
        """ Used to request for heal of From-VNF """
        LOG.debug("__heal_from_vnf(self, reason) - reason: %s" % (reason))
        try:
            heal_rsp_data = self.vnfm_comm_handle.heal_vnf(self.upgrade_job.get_vnf_id(),
                                                     reason)
            ...
            self.__wait_for_lc_op_ok(heal_rsp_data['vnfLcOpId'],
                ...
                vnfm_api_comm.VnfmCommException) as exception:
            LOG.warning("__heal_from_vnf(self). The requested heal reported failure for "
                        "From-VNF (%s). I.e. it might end up in that this VNF not will "
                        "be fully functional. This problem is ignored since nothing more"
    ...
    def __cancel_from_vnf(self):
        ...
        if (self.working_state == EnumWorkingState.WAIT_FOR_CREATE_BACKUP_RESPONSE or
            self.working_state == EnumWorkingState.WAIT_FOR_EXPORT_BACKUP_RESPONSE):
            try:
    ...
    def __rollback_at_restore(self, add_info = list()):
        ...
        #TODO_J: Diff vs activate_cinder.py ?
    ...
    def __copy_esi(self):
        """ Copy ESI from From-VNF to To-VNF """
        LOG.debug("__copy_esi(self)")
        export_result = self.__export_esi(self.vnf_comm_handle_from,
                                          self.upgrade_job.get_vnf_id(),
                                          self.ESI_FILE_NAME,
                                          "Requesting From-VNF to export ESI to tmp directory")
        self.__import_esi(self.__get_vnf_comm_to(),
                          self.upgrade_job.get_to_vnf_id(),
                          export_result,
                          "Requesting To-VNF to import ESI (origin from From-VNF)")
