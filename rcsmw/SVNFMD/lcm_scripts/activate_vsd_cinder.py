...
LOG = logging.getLogger('ACTIVATE_VSD_CINDER')
...
class Activate(lcm_wf_utility.GenAsyncQueue):
    ...
    def handle_fetch_upgrade_log_result(self, result):
        """
        ...
        try:
            if result['result'] == 'SUCCESS':
                ...
                self.__get_vnf_comm_to().start_traffic_v2(self.stop_traffic_data)
                self.__is_cancel_requested()
    ...
    def __continue_activation(self, upg_operation_data):
        ...
        if (upg_operation_data['vnf_id'] == self.upgrade_job.get_to_vnf_id()):
            ...
            return self.handle_fetch_upgrade_log_result(result)
    ...
    def __handle_create_backup_response(self, operation_data):
        ...
        self.stop_traffic_data = self.vnf_comm_handle_from.stop_traffic_v2(self.requested_time_for_vnf,
                                                                           "UpgradeNormal")
        ...
        vnf_info = self.vnfm_comm_handle.get_vnf_info(
            ...
            [vnfm_api_comm.VnfmComm.EXT_VIRTUAL_LINKS,
             ...
             vnfm_api_comm.VnfmComm.EXT_CPS])
    ...
    def __instantiate_vnf(self, instance_name,  description, ext_virtual_links, storages, ext_cps):
        """ Instantiates a VNF """
        LOG.debug("__instantiate_vnf(self, instance_name,  description, ext_virtual_links, "
                  "storages, ext_cps) - instance_name: %s,  description: %s, "
                  "ext_virtual_links: %s, storages: %s, ext_cps: %s"
                  % (instance_name,  description, ext_virtual_links, storages, ext_cps))
        ...
        http_payload = self.__add_ext_virtual_links(ext_virtual_links, ext_cps, http_payload)
    ...
    def __add_storage_heal(self, storage, payload_dict):
        ...
        payload_dict["additionalParams"] = list()
    ...
    def __add_fronthaul_port_for_from_vnf(self, http_payload):
        """ Add From-VNF fronthault port id to heal request """
        LOG.debug("__add_fronthaul_port_for_from_vnf(self, http_payload)"
                  " - http_payload: %s" % (http_payload))
        if 'additionalParams' not in http_payload:
            http_payload['additionalParams'] = list()
        http_payload["additionalParams"].append({"ports": self.FRONTHAUL_PORT})
        http_payload["additionalParams"].append({self.FRONTHAUL_PORT:
                                                 self.upgrade_job.resource.get_ext_cp_port_id_from_vnf()})
        LOG.debug("__add_fronthaul_port_for_from_vnf(...) - RETURNS http_payload: %s"
                  % (http_payload))
        return http_payload
    ...
    def __add_ext_virtual_links(self, ext_virtual_links, ext_cps, http_payload):
    ...
    def __extract_cpd_id(self, ext_virtual_link_ext_cps, ext_cps):
        ...
        for ext_virt_link_data in ext_virtual_link_ext_cps:
            port_name = ext_virt_link_data['cpdId']
            if port_name == self.FRONTHAUL_PORT:
                ext_cp = self.__get_ext_cp_for_port_name(port_name, ext_cps)
                ext_virtual_links.append({'cpdId': ext_virt_link_data['cpdId'],
                                          'addresses': self.__get_ip_addresses_from(ext_cp)})
                self.upgrade_job.resource.set_ext_cp_port_id_from_vnf(
                    self.__get_resource_id_from(ext_cp))
                self.__set_resource()
            else:
                ext_virtual_links.append({'cpdId': ext_virt_link_data['cpdId']})
    ...
    def __get_ip_addresses_for_port(self, port_name, ext_cps):
        """ Returns all ip addresses used for given port name """
        LOG.debug("__get_ip_addresses_for_port(self, port_name, ext_cps) - "
                  "port_name: %s, ext_cps: %s" % (port_name, ext_cps))

        ip_addresses = []
        for ext_cp in ext_cps:
            if port_name == ext_cp.cpd_id:
                ip_addresses = ext_cp.ip_addresses
                break

        return ip_addresses

    def __get_ip_addresses_from(self, ext_cp):
        """ Returns all ip addresses used for given port name """
        LOG.debug("__get_ip_addresses_from(self, ext_cp) - "
                  "ext_cp: %s" % (ext_cp))
        ...
        LOG.debug("__get_ip_addresses_from(...) - RETURNS ip_addresses: %s" % (ip_addresses))
        return ip_addresses

    def __get_resource_id_from(self, ext_cp):
        """ Returns the resource id (port id) for given ext cp """
        LOG.debug("__get_resource_id_from(self, ext_cp) - "
                  "ext_cp: %s" % (ext_cp))

        resource_id = None
        if ext_cp != None:
            resource_id = ext_cp.resource_id

        LOG.debug("__get_resource_id_from(...) - RETURNS resource_id: %s" % (resource_id))
        return resource_id

    def __get_ext_cp_for_port_name(self, port_name, ext_cps):
        """ Returns ext_cp for given port name, otherwise None """
        LOG.debug("__get_ext_cp_for_port_name(self, port_name, ext_cps) - "
                  "port_name: %s, ext_cps: %s" % (port_name, ext_cps))
        the_ext_cp = None
        for ext_cp in ext_cps:
            if port_name == ext_cp.cpd_id:
        ...
        LOG.debug("__get_ext_cp_for_port_name(...) - RETURNS - the_ext_cp: %s"
                  % (the_ext_cp))
        return the_ext_cp
    ...
    def __heal_from_vnf(self, reason, to_vnf_id):
        ...
        if to_vnf_id != None:
            # Add the ip addresses for To-VNF fronthaul_port to heal request
            http_payload = self.__add_fronthaul_port_for_from_vnf(http_payload)
            http_payload['cause'] = 'rollback'

        if 'cause' not in http_payload:
            http_payload['cause'] = reason
