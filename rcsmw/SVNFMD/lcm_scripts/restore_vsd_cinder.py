...
LOG = logging.getLogger('RESTORE_VSD_CINDER')
...
class Restore(lcm_wf_utility.GenAsyncQueue):
    ...
    def handle_main(self, operation_data):
        ...
        try:
            ...
            self.stop_traffic_data = self.__stop_traffic(self.requested_time_for_vnf,
                                                         "SoftwareRestore")
    ...
    def __continue_restore(self, operation_data):
        ...
        if (operation_data['vnf_id'] == self.__get_restore_job().get_to_vnf_id()):
            ...
            if self.stop_traffic_data == None:
                self.stop_traffic_data = {'stop_time': from_vnf_shutdown_time}
                self.add_infos.append("Using shutdown request time of From-VNF as "
                                      "stop traffic data (%s)" %
                                      (from_vnf_shutdown_time))
            self.__start_traffic(self.stop_traffic_data)
    ...
    def __instantiate_vnf(self, instance_name,  description, ext_virtual_links, storages, ext_cps):
        ...
        LOG.debug("__instantiate_vnf(self, instance_name,  description, ext_virtual_links, "
                  "storages, ext_cps) - instance_name: %s,  description: %s, ext_virtual_links:"
                  " %s, storages: %s, ext_cps: %s"
    ...
    def __add_ext_virtual_links(self, ext_virtual_links, ext_cps, http_payload):
        ...
        LOG.debug("__add_ext_virtual_links(self, ext_virtual_links, ext_cps, http_payload)"
                  " - ext_virtual_links: %s, ext_cps: %s, http_payload: ***" %
                  (ext_virtual_links, ext_cps))
    ...
    def __extract_cpd_id(self, ext_virtual_link_ext_cps, ext_cps):
        ...
        LOG.debug("__extract_cpd_id(self, ext_virtual_link_ext_cps, ext_cps) - "
                  "ext_virtual_link_ext_cps: %s, ext_cps: %s" %
                  (ext_virtual_link_ext_cps, ext_cps))
        ...
        for ext_virt_link_data in ext_virtual_link_ext_cps:
            ...
            if port_name == self.FRONTHAUL_PORT:
                ...
                self.__get_restore_job().resource.set_ext_cp_port_id_from_vnf(
    ...
    def __act_on_heal_operation_indication(self, vnf_lccn_operation_data):
        ...
        if (vnf_lccn_operation_data['status'] == 'success' or
            ...
            reason = ("A heal of To-VNF (%s) has been requested when not expected. "
                      ...
                      "The restore will be rolled back due to this."
    ...
    def __stop_traffic(self, time_of_action, cause):
        ...
        try:
            # Is used as indication that stop traffic has been requested
