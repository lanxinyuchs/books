...
%%% @version /main/R1A/R2A/R3A/R4A/R5A/R6A/R8A/R9A/R11A/3
%%%
...
-module(swmAppData).
-vsn('/main/R1A/R2A/R3A/R4A/R5A/R6A/R8A/R9A/R11A/3').
-date('2017-10-18').
-author('etxberb').
%%% ----------------------------------------------------------
...
%%% ----    ---------- -------  ------------------------------------------------
%%% R8A/1   2016-12-06 erarafo  Using swmI functions
%%% ----    ---------- -------  ------------------------------------------------
%%% R9A/1   2017-04-20 etxberb  Added prepPush_appdata_single/1.
%%% R9A/2   2017-04-24 etxberb  Cleaning up appdata_dir after move_appdata.
%%% R9A/3   2017-04-27 etxberb  Added unregisteredCxps.
%%% R9A/5   2017-05-09 etxberb  Correction of unregisteredCxps.
%%% R9A/6   2017-05-15 etxberb  Added registerCxps/3.
%%% ----    ---------- -------  ------------------------------------------------
%%% R11A/2  2017-10-12 etxberb  Fixed dialyzer problem.
%%% ----------------------------------------------------------
%%% 
%%% #2.    EXPORT LISTS
%%% ----------------------------------------------------------
...
-export([prepPush_appdata_single/1]).
...
prepare_appdata(AdPhase, SwFolder, ToFolder, CxpMatch) ->
    erlang:spawn(?MODULE,
		 init_prepare_appdata,
		 [AdPhase, SwFolder, ToFolder, self(), CxpMatch]),
    receive
...
%%% ###########################################################################
%%% @doc Find all cxp metadata files named cxp*.xml for a single CXP and 
%%%   parse them for appdata information.
%%%
%%% @end
%%% ----------------------------------------------------------
-spec prepPush_appdata_single(CxpId :: string()) ->
    ok | nok.
%%% ###=====================================================================###
prepPush_appdata_single(CxpId) ->
    ?LOG_INFO([{cxpId, CxpId}]),
    AdPhase = appdata,
    SwFolder = swmLib:software_dir(),
    ToFolder = ?tmp_appdata_dir,
    prepare_appdata(AdPhase, SwFolder, ToFolder, CxpId),
    push_appdata().
...
init_prepare_appdata(AdPhase, SwFolder, ToFolder, Parent, CxpMatch) ->
...
supervise_prepare_appdata(AdPhase, SwFolder, ToFolder, Parent, T0, CxpMatch) ->
	...
	begin
	    ...
	    os:cmd(["rm -rf ", ?tmp_appdata_dir]),
	    os:cmd(["rm -rf ", ?tmp_appdata_cxp_dir]),
	    file:make_dir(?tmp_appdata_dir),
	    file:make_dir(?tmp_appdata_cxp_dir),
	    do_prepare_appdata(AdPhase, SwFolder, ToFolder, T0, CxpMatch),
	    wait4push(AdPhase, ToFolder)
	end
    catch
	ErrClass : ErrReason ->
	    ?LOG_ERR([{adPhase, AdPhase},
		      {swFolder, SwFolder},
		      {toFolder, ToFolder},
		      {ErrClass, ErrReason},
...
do_prepare_appdata(appdata_upgrade = AdPhase,
		   SwFolder,
		   ToFolder,
		   T0,
		   CxpMatch) ->
    ...
    put(fd, Fd),
    parseCxp_copyCxc_appdata(AdPhase, SwFolder, ToFolder, T0, CxpMatch),
    file:close(Fd),
    erase(fd);
do_prepare_appdata(appdata = AdPhase, SwFolder, ToFolder, T0, CxpMatch) ->
	...
	{ok, [{_, _, _, _} | _] = Data} ->
	    ...
	    RegCxps = registerCxps(AdPhase, SwFolder, CxpMatch),
	    ?LOG_INFO([{regCxps, RegCxps}]),
	...
	{error, enoent} ->
	    ...
	    parseCxp_copyCxc_appdata(AdPhase, SwFolder, ToFolder, T0, CxpMatch);
	Other ->
	    ?LOG_WARN([{file, consult, [IndexFile]},
		       Other]),
	    parseCxp_copyCxc_appdata(AdPhase, SwFolder, ToFolder, T0, CxpMatch)
    end.
...
move_appdata(T1, T0) ->
    ...
    CxcCnt = length(Files),
    os:cmd(["rm -rf ", swmLib:appdata_dir()]),
    ?LOG_INFO([{indexFile2ets, sysUtil:time_to_string(T1 - T0)},
	       {{move_rcs2tmp, CxcCnt}, sysUtil:time_to_string(T2 - T1)},
...
parseCxp_copyCxc_appdata(AdPhase, SwFolder, ToFolder, T0, CxpMatch) ->
    ...
    CxpPattern = filename:join([SwFolder, CxpMatch, "cxp*.xml"]),
    CxpFiles = unregisteredCxps(AdPhase, filelib:wildcard(CxpPattern)),
    ?LOG_INFO(ets:tab2list(swmAppdataReg)),
    ?LOG_INFO([{cxpPattern, CxpPattern}
	       | [{filename:basename(filename:dirname(CxpFile)),
		   filename:basename(CxpFile)}
		  || CxpFile <- CxpFiles]]),
    T1 = ?MonoTime,
    ...
    ?LOG_INFO([{prepare, sysUtil:time_to_string(T1 - T0)},
	       {{copy2tmp_cxp, CxpCnt}, sysUtil:time_to_string(T2 - T1)},
	       {{copy2tmp_cxc, CxcCnt}, sysUtil:time_to_string(T3 - T2)},
	       {'TOTAL', sysUtil:time_to_string(T3 - T0)}]),
    ok.

registerCxps(appdata, SwFolder, CxpMatch) ->
    CxpPattern = filename:join([SwFolder, CxpMatch, "cxp*.xml"]),
    [begin
	 Swp = filename:basename(filename:dirname(CxpFile)),
	 mnesia:dirty_write(#swmAppdataReg{swp = Swp}),
	 Swp
     end
     || CxpFile <- filelib:wildcard(CxpPattern)].

unregisteredCxps(appdata_upgrade, CxpFiles) ->
    CxpFiles;
unregisteredCxps(_, CxpFiles) ->
    unregisteredCxps(CxpFiles).

unregisteredCxps([CxpFile | Tail]) ->
    Swp = filename:basename(filename:dirname(CxpFile)),
    case mnesia:dirty_read(swmAppdataReg, Swp) of
	[#swmAppdataReg{is_registered = true}] ->
	    ?LOG_INFO([{"Already registered", Swp}]),
	    unregisteredCxps(Tail);
	_ ->
	    mnesia:dirty_write(#swmAppdataReg{swp = Swp}),
	    [CxpFile | unregisteredCxps(Tail)]
    end;
unregisteredCxps([]) ->
    [].

copy_cxpAppdata_tmp([CxpFile | Tail]) ->
    ActualFile = swmI:find_file(CxpFile),
    try
  ...
  when ProductE#xmlElement.name == product ->
    [begin
	 ...
	 try find_attribute(file, AppDataE) of
	     File ->
		 ActualFile =
		     swmI:find_file(filename:join([CxpDir, RelPath, File])),
		 ...
		 [begin
		      ActualFile =
			  swmI:find_file(filename:join([CxpDir,
							  RelPath,
							  File])),
		      store_zip(ActualFile, ToFolder, ProdId, Version)
		  end
...
call_appdata_func(Module, Func, Args, ActualFile) ->
	...
	error:undef ->
	    case Func of
		appdata ->
		    ?LOG_ERR([{Module, Func, Args} | mod_inf(Module)] ++
			     ["--- Stacktrace ---" | erlang:get_stacktrace()]),
