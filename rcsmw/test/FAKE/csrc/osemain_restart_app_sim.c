...
start_OSE(void)
{
  ...
  if ( (ptr = getenv("ARGC")) != NULL )
     argc = strtoul(ptr, NULL, 0);

  if ( (ptr = getenv("ARGV")) != NULL )
...
{
  char str[2*sizeof(unsigned long) + 3];
  ...
  */
  snprintf(str, sizeof(str), "0x%lx", (unsigned long)argc);
  setenv("ARGC", str, 1);
  snprintf(str, sizeof(str), "0x%lx", (unsigned long)argv);
  setenv("ARGV", str, 1);
