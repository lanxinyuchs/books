...
elif [ "$(cat /vobs/rcs/test/RCT_CRX901275/test/jenkins/${BRANCH_INFO} | grep "translation:$(basename $j):" )" != "" ]; then
   TE_ST=$(cat /vobs/rcs/test/RCT_CRX901275/test/jenkins/${BRANCH_INFO} | grep "translation:$(basename $j):" | cut -f3- -d: )
else
   TE_ST=$(basename $j)
...
##############################################################################################
# NOTE!  Bad solution to create a link to jenkins graph 
#
    if [ "$a" = "test_activity_XXL_weekly" ]; then
       LATEST_JOB_NO=$(cat $i | cut -f2- -d# | cut -f1 -d\<)
       LATEST_JENKINS_NO=$(ls -rt $JENKINSJOBS/config_xxl_mem_check_150hour_nc_cli_operation_spec_dus41/builds/ | grep -v "-" | tail -1)
       if [ "$LATEST_JOB_NO" = "$LATEST_JENKINS_NO" ]; then
         aa="<a href=\"https://rbs-rde-ci.rnd.ki.sw.ericsson.se/view/statistics/job/plot_xxl_test_1/plot/getPlot?index=0&width=2000&height=800\"><font color=black>${a}</font> <font color=green><blink pars=0>plot</blink></font></a>"
       else
         aa=$a
       fi
    else
...
##############################################################################################  
   # echo "<td > $aa</td>" 
   # echo "<td> $(cat $RESULTDIR/$LSV_NO/$a.new_state)</td>" 
   # echo "<td> $s</td>"
   # echo "<td> $(cat $activity/$s.new_state)</td>"
    Qtime="-"
    Etime="-"
    Ttime="-"
...
REQUIRED=$(cat /vobs/rcs/test/RCT_CRX901275/test/jenkins/${BRANCH_INFO} | grep "stable_label:" | grep -v "#" | cut -f2- -d: )
fi
COLOR10="#CC99FF"
...
for j in $ACIVITYDIR
do
  if [ "$(echo $REQUIRED | grep $(basename $j).new_state)" = "" ] && [ "$(echo $basename $j | grep "_XL_")" = "" ] && [ "$(echo $basename $j | grep "sbt_jcat")" = "" ] && [ "$(echo $basename $j | grep "release_candidate")" = "" ]  && [ "$(echo $basename $j | grep "qualification")" = "" ]; then
    if [ "$(echo $basename $j | grep "jcat")" != "" ] ; then
       print_table_jcat
    else
    print_table
...
if [ "$(echo $ACIVITYDIR | grep "_XL_")" != "" ] || [ "$(echo $ACIVITYDIR | grep "_XXL_")" != "" ] || [ "$(echo $ACIVITYDIR | grep "test_activity_sbt_jcat")" != "" ] || [ "$(echo $ACIVITYDIR | grep "test_activity_jcat_oss_sbt_jcat")" != "" ]; then
...
do
  ...
  elif [ "$(echo $basename $j | grep "_XL_")" != "" ] || [ "$(echo $basename $j | grep "_XXL_")" != "" ]; then
    print_table
  fi
done
fi
...
###################################################################################
if [ "$(echo $ACIVITYDIR | grep "release_candidate")" != "" ]; then
echo "<th bgcolor=\"${COLOR10}\" colspan=\"8\"><b> SBC release candidate tests </b></th>"
echo "</tr>" 
result_head
for j in $ACIVITYDIR
do
  if [ "$(echo $basename $j | grep "release_candidate")" != "" ]; then
    print_table
  fi
done
fi
...
echo "<td bgcolor=\"#C0C0C0\" colspan=\"2\"><b>All Results</b></td>"
echo "<td bgcolor=\"#C0C0C0\" align=\"right\"><b> $TTFAILED </b></td>"
echo "<td bgcolor=\"#C0C0C0\" align=\"right\"><b> $TTSKIPPED </b></td>"
echo "<td bgcolor=\"#C0C0C0\" align=\"right\"><b> $TTTOTAL </b></td>"
echo "<td bgcolor=\"#C0C0C0\" align=\"right\"> $TTQtime </td>"
...
echo "</table>"
echo "<table width=\"%95\" border=\"1\">"
echo "<p>Common Test State ( <span style=\"color:black\">not started </span><span style=\"color:blue\">running </span><span style=\"color:green\">success </span><span style=\"color:orange\">unstable </span><span style=\"color:red\">failed </span>)</p>"

CXS=$(cat $lsvnodir/TestCs | awk '{print $3}' | cut -f1 -d\<)
CXS=$(basename $(cat $lsvnodir/TestCs | cut -f2 -d\" ))
if [ "$(ls /proj/rcs-tmp/stps/*/${CXS}/ | grep TESTUSER)" != "" ]; then
if [ "$(ls /proj/rcs-tmp/stps/*/${CXS}/TESTUSER/ | grep rcsci1)" != "" ]; then
LOGSDIR=$(ls -d /proj/rcs-tmp/stps/*/$CXS/TESTUSER/rcsci1/*/* | grep -v "VER_"  | grep -v "CNX" | grep -v "pre_dc_ee" )

if [ "$LOGSDIR" != "" ]; then
LOGSDIR=$(ls -d /proj/rcs-tmp/stps/*/$CXS/TESTUSER/rcsci1/* | grep -v "VER_"  | grep -v "CNX" | grep -v "pre_dc_ee" )
echo "<table width=\"%20\" border=\"1\">"
echo "<tr>"
echo "<th colspan=\"5\"><b><center> Troubleshooting<blink pars=0>!</blink> <center></b></th>"
echo "<tr>"
echo "<th bgcolor=\"#C0C0C0\"><b>STP</b></th>"
echo "<th bgcolor=\"#C0C0C0\"><b> Common Test Log</b></th>"
echo "<th bgcolor=\"#C0C0C0\"><b> Fail </b></th>"
echo "<th bgcolor=\"#C0C0C0\"><b> Skip </b></th>"
echo "<th bgcolor=\"#C0C0C0\"><b> Total </b></th>"
echo "</tr>"
for i in $LOGSDIR
do
if [ "$(ls $i | grep ct_run)" != "" ]; then
STP=$(echo $i | cut -f5 -d\/)


COMPLETE="0"
RUNNING="0"
if [ "$( grep  ct_run $i/* )" != "" ]; then
COMPLETE=$(cat $i/ct_run.*/*suites.*.*.logs/*/suite.log | grep "=== TEST COMPLETE" | wc -l )
RUNNING=$(ls -d $i/ct_run.*/*sui*.logs | wc -l)
fi

TST="0"
TSS="0"
TSF="0"
for j in $(cat $i/ct_run.*/*suites.*.*.logs/*/suite.summary )
do
 A=$(echo $j | cut -f3 -d\{ | cut -f1 -d\}  | cut -f1 -d,)
 B=$(echo $j | cut -f3 -d\{ | cut -f1 -d\}  | cut -f2 -d,)
 C=$(echo $j | cut -f3 -d\{ | cut -f1 -d\}  | cut -f3 -d,)
 D=$(echo $j | cut -f3 -d\{ | cut -f1 -d\}  | cut -f4 -d,)
TST=$( expr $TST + $A + $B + $C + $D )
TSF=$( expr $TSF + $B )
TSS=$( expr $TSS + $C + $D )
done

#{summary,{2,0,0,0}}.
if [ "$COMPLETE" = "$RUNNING" ] && [ "$TST" = "0" ] && [ "$TSF" = "0" ] && [ "$TSS" = "0" ]; then
COLOR="blue"
elif [ "$COMPLETE" = "$RUNNING" ] && [ "$TSF" = "0" ] && [ "$TSS" = "0" ]; then
COLOR="green"
elif [ "$COMPLETE" = "$RUNNING" ]; then
COLOR="red"
else
COLOR="blue"
fi
echo "<tr>"
echo "<td> <a href=\"https://rbs-rde-ci.rnd.ki.sw.ericsson.se:443/computer/${STP}\"<font color=black>${STP}</font></a></td>"
echo "<td > <a href=\"https://rbs-rde.rnd.ki.sw.ericsson.se/$i/\"> <font color=${COLOR}>$(basename $i) </font></a></td>"
echo "<td > ${TSF} </td>"
echo "<td > ${TSS} </td>"
echo "<td > ${TST} </td>"
echo "</tr>"
...
############################################################################################################
LOGSDIR=$(ls -d /proj/rcs-tmp/stps/*/$CXS/TESTUSER/rcsci1/*/* | grep "pre_dc_ee" )

if [ "$LOGSDIR" != "" ]; then
echo "<table width=\"%40\" border=\"1\">"
echo "<tr>"
echo "<th colspan=\"6\"><b><center> PRE DC EE <blink pars=0>!</blink> <center></b></th>"
echo "<tr>"
echo "<th bgcolor=\"#C0C0C0\"><b>VER</b></th>"
echo "<th bgcolor=\"#C0C0C0\"><b>STP</b></th>"
echo "<th bgcolor=\"#C0C0C0\"><b> Common Test Log</b></th>"
echo "<th bgcolor=\"#C0C0C0\"><b> Fail </b></th>"
echo "<th bgcolor=\"#C0C0C0\"><b> Skip </b></th>"
...
echo "<td > ${VER} </td>"
echo "<td> <a href=\"https://rbs-rde-ci.rnd.ki.sw.ericsson.se:443/computer/${STP}\"<font color=black>${STP}</font></a></td>"
echo "<td > <a href=\"https://rbs-rde.rnd.ki.sw.ericsson.se/$i/\"> <font color=${COLOR}>$(basename $(dirname $i)) </font></a></td>"
echo "<td > ${TSF} </td>"
echo "<td > ${TSS} </td>"
echo "<td > ${TST} </td>"
echo "</tr>"
...
############################################################################################################
LOGSDIR=$(ls -d /proj/rcs-tmp/stps/*/$CXS/TESTUSER/rcsci1/*/* | grep "VER_" )

if [ "$LOGSDIR" != "" ]; then
echo "<table width=\"%40\" border=\"1\">"
echo "<tr>"
echo "<th colspan=\"6\"><b><center> Troubleshooting on modified UP <blink pars=0>!</blink> <center></b></th>"
echo "<tr>"
echo "<th bgcolor=\"#C0C0C0\"><b>VER</b></th>"
echo "<th bgcolor=\"#C0C0C0\"><b>STP</b></th>"
echo "<th bgcolor=\"#C0C0C0\"><b> Common Test Log</b></th>"
