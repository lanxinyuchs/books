...
%%%%%%%%%%%%%%%%%%%%%% ftpTestLib.hrl => ftpes_test_lib.hrl %%%%%%%%%%%%%%%%%%%%%
%%% R8A/1    2017-01-17   ekurnik    Moved file to RCT block from FTPES
%%% R9A/1    2017-04-05   ekurnik    FTPES server port is fetched from node
%%%--------------------------------------------------------------------
%%% R12A/1   2017-11-08   ekurnik    Added defines to support rct_mo_handler usage
%%% R12A/2   2017-11-30   edartop    Added port range in ftpDataConnection
%%%--------------------------------------------------------------------
...
-define (DEFAULT_SERVER_CONTROL_CONNECTION_PORT, 
            case rct_rpc:call(rpc, ftpesServer, get_ftpes_port_conf, [], 5000) of
                [{server_control_port, Port} | _] ->
                    Port;
                _ ->
                    9921
            end
...
-define(FTP_SERVER_DN, "ManagedElement=1,SystemFunctions=1,SysM=1,FileTPM=1,FtpServer=1").
-define(FTP_TLS_DN, "ManagedElement=1,SystemFunctions=1,SysM=1,FileTPM=1,FtpTls=1").
-define(FTP_TLS_SERVER_DN, ?FTP_TLS_SERVER_DN('FileTPM_1_1')).
-define(FTP_TLS_SERVER_DN(FileTPM_version), case FileTPM_version of
                                                'FileTPM_1_1' -> "ManagedElement=1,SystemFunctions=1,SysM=1,FileTPM=1,FtpTls=1,FtpTlsServer=1";
                                                'FileTPM_2_0' -> "ManagedElement=1,SystemFunctions=1,SysM=1,FileTPM=1,FtpServer=1,FtpTlsServer=1"
                                            end).

-define(ADDRESS_MO_DN(Type, IpProto), get_oam_addr_ref(Type, IpProto)).
-define(OAM_ACCESS_POINT_DN(Type), "ManagedElement=1,SystemFunctions=1,SysM=1,OamAccessPoint=" ++ ?OAM_ID(Type)).
-define(OAM_ID(Type), case Type of
                          oam -> "1";
                          alt_oam -> "Alternative"
                      end).

-define(MO_HANDLER, {netconf, nc1}).
-define(MO_HANDLER(NC), {netconf, NC}).
