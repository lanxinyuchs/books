%%% #0.    BASIC INFORMATION
%%% ----------------------------------------------------------
%%% %CCaseFile:	rct_cli.erl %
%%% @author etxkols
%%% @copyright Ericsson AB 2012-2016
%%% @version /main/R1A/R2A/R3A/R4A/R5A/4
%%%
%%% @doc ==Common Test hook for sending and receiving cli messages==
%%% 
...
%%% 
%%% Hook formats:
%%% ```{rct_cli, [{N, Name, IPType, Opts}]}'''
%%%    Hook short formats
%%% ```{rct_cli, Name}                           -> {rct_cli, [{1, Name, oam_auto, []}]}
%%%    {rct_cli, {Name, Opts}                    -> {rct_cli, [{1, Name, oam_auto, Opts}]}}
%%%    {rct_cli, [Name1, Name2]                  -> {rct_cli, [{1, Name1, oam_auto, []},{2, Name2,oam_auto , []}]}}
%%%    {rct_cli, [{Name1, Opts1},{Name2, Opts2}] -> {rct_cli, [{1, Name1, oam_auto, Opts1},{2, Name2, oam_auto, Opts2}]}}'''
%%%
...
%%%    IPType   =                                Used in target env to specify which IP address cli uses,
...
%%%    manual_connect                            No automatic connection to CLI at start of testcase, i.e. use rct_cli:connect.
%%%    ConnectTimeout                            default = 20 seconds, Intended for TLM'''
%%%
%%% Before each suite `pre_init_per_suite' will:<br/>
%%% - Verify existance of config parameter `{rct_cli, [{N, Name, Sname, IPType, Opts}]}'.<br/>
...
%%%
%%% ```rct_run.sh -stp dus5000                  will run cli commands over ssh_lmt_ipv4
%%%    rct_run.sh -stp dus5000 -oamap_ipv4      will run cli commands over ssh_TN_A_ipv4
%%%    rct_run.sh -stp dus5000 -oamap_ipv6      will run cli commands over ssh_TN_A_ipv6'''
%%%
...
%%% ```suite() -> 
%%%        [{ct_hooks, [{rct_cli, cli}]}].'''
%%% ```suite() -> 
%%%        [{ct_hooks, [{rct_cli, {cli_1,[manual_connect]}}]}].'''
%%% Examples clustered node:
%%% ```suite() -> 
%%%        [{ct_hooks, [{rct_cli, [cli_1, cli2]}]}].'''
%%%
%%% Testcase example.
%%% Send a cli cmd.
%%% ```cli_ex(_) ->
%%%        rct_cli:send(cli_1, "configure").'''
%%%
%%% Send a cli cmd and match a expected string in the received data.
%%% ```cli_ex(_) ->
%%%        rct_cli:send(cli_1, "configure", "\\(config\\)>").'''
%%%
%%% Send a cli cmd and get the recieved data.
%%% ```cli_ex(_) ->
%%%        {ok, {RecData, Match}} = rct_cli:send(cli_1, "configure", "\\(config\\)>").'''
%%%
%%% @end

-module(rct_cli). 
-id('Updated by CCase').
-vsn('/main/R1A/R2A/R3A/R4A/R5A/4').
-date('2016-02-25').
-author('etxkols').
...
%% Rev        Date        Name        What
%% -----      ---------   --------    ------------------------
%% R1A/1      2012-08-9   etxivri    Created
%% R1A/2      2012-08-14  etxivri    Minor edoc correction.
%% R1A/3      2012-10-05  etxivri    Minor change in get_cli_port when using sim env.
%% R1A/4      2012-10-17  etxkols    Major changes to fit into Jenkins short_test.
%% R1A/5      2012-10-18  etxkols    Edoc update and robustness fixes
%% R2A/4      2012-10-19  etxkols    Use rct_cli_coli_common
%% R2A/5      2014-03-26  etxkols    Faulty init/2 return value
%% R2A/6      2014-04-23  etxkols    Added connect_timeout for TLM
%% R4A/1      2015-05-28  etxkols    Fixed cluster
%% R4A/1      2015-10-12  uabesvi    added asend
...
%% @doc Disconnects ssh cli port from SUT.<br/>
...
%% @doc terminates ssh cli hook<br/>
terminate(States) ->
...
%%    Print = print | noprint    sent commands and replies are printed in html. Overrides ct_hook setting.
%%    Timeout = integer()        Seconds to wait for prompt, default 5 seconds.
%%    Reply = term()
...
%% @doc asynchroneous version of send/2. see asend/3
asend(Name, Cmd) ->
    rct_cli_coli_common:asend(Name, Cmd).
%% @spec asend(Name, Cmd, Print) -> {ok, {Reply, Matched}} | {error, Reason}
%% @doc Runs CLI command without waiting for result result, Print overrides ct_hook setting.
%%
%% For description of Match, please see <a href="http://www.erlang.org/doc/man/re.html#run-3">re:run/3</a>.
%% ```Name = atom()              Used as identifier.
%%    Cmd = string()             cli command you want to send.
...
asend(Name, Cmd, Print) when is_atom(Print) ->
    rct_cli_coli_common:asend(Name, Cmd, Print).
...
%% @spec get_port(Name) -> {ok, Port} | {error, Reason}
%% @doc Get CLI port 
%% ```Name = atom()              Used as identifier.
%%    Port = integer()
%%    Reason = term()'''
get_port(Name) ->
    rct_cli_coli_common:get_port(Name, ?CLI_OR_COLI, ?TARGET_PORT).
