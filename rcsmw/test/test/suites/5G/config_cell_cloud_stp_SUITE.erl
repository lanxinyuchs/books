...
-module(config_cell_cloud_stp_SUITE).
...
-export([suite/0,
	 ...
	 end_per_testcase/2,
	 all/0,
	 configure_cell_cloud_stp_ipv6/1,
	 configure_cell_cloud_stp_ipv4/1
	]).
...
suite() -> 
    [{ct_hooks, [{rct_node_state, [{1,vnfm,up},{2,vnf,up},{3,vnf,up},{4,du,up}]},%%vnfm,vrc, sd and bpu
		 {rct_cli, [{cli1_vnfm, [manual_connect]},{cli1_vrc, [manual_connect]},
		  	    {cli1_sd, [manual_connect]},{cli1_bpu, [manual_connect]}]},
		 {rct_coli,[ {coli1_vnfm, [manual_connect]}, {coli1_vrc, [manual_connect]}, 
			     {coli1_sd, [manual_connect]}, {coli1_dus, [manual_connect]}]},
		 ...
		 {rct_netconf, [nc_vnfm1, nc_vrcs1, nc_sd1, nc_bpu1]}
...
init_per_suite(Config) ->
    ...
    ct:pal("Hw BPU: ~p", [HW_BPU1]),
   
    [{hw_sd,HW_SD},{hw_vrcs1,HW_VRCS1},
     {hw_bpu1,HW_BPU1} | Config]. 
...
configure_cell_cloud_stp_ipv6(Config)->
    IpVersion = "6",
    ...
    config_cloud_stp_lib:deblock_cell(nc_vrcs1,Hw_vrc1),
    %%Check XCM connection
    %% check_ip_registrate_sd(coli1_vrc, SdIP)

    ok.

configure_cell_cloud_stp_ipv4(Config)->
