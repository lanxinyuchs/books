%%% #0.    BASIC INFORMATION
%%% ----------------------------------------------------------
%%% %CCaseFile:	ai_backup_SUITE.erl %
%%% @author etxmlar
%%% @copyright Ericsson AB 2015-2016
%%% @version /main/R3A/R4A/R6A/R7A/2
%%%
%%% @doc == AI using backup.==
%%%
%%%
...
-module(ai_backup_SUITE).
-author('etxmlar').
-vsn('/main/R3A/R4A/R6A/R7A/2').
-date('2016-11-15').
...
%%% Rev        Date       Name        What
%%% -----      ---------  --------    ------------------------
%%% R3A/2      2015-05-06 etxivri     Created
%%% R3A/3      2015-06-02 etxivri     Remove hardcoded sftp server data.
%%% R4A/1      2015-07-01 etxivri     Make it more robust
%%% R7A/1      2016-10-24 etxmlar     Added support for UP .zip
...
end_per_testcase(_TestCase, Config) ->
    ...
    %% If CXS UP rm 
    %%os:cmd("rm "++?TFTPBOOT_DIR++NodeName++"/_"++?BU_NAME++"_*"),
    %% if ZIP UP rm 
    %%os:cmd("rm "++?TFTPBOOT_DIR++NodeName++"/"++?BU_NAME++"_*"),
...
ai_load_backup(Config) ->
    ct:log("TC: C_3, AI usisng backup"),
    NodeName = proplists:get_value(nodename, Config),
