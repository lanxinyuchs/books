%%% #0.    BASIC INFORMATION
%%% ----------------------------------------------------------
%%% %CCaseFile:	ai_user_interface_4_SUITE.erl %
%%% @author etxmlar
%%% @copyright Ericsson AB 2015-2017
%%% @version /main/R4A/R7A/R9A/2
%%%
...
-module(ai_user_interface_4_SUITE).
-author('etxmlar').
-vsn('/main/R4A/R7A/R9A/2').
-date('2017-04-19').
...
init_per_testcase(TestCase = c_1_6, Config) ->
...
%%%--------------------------------------------------------------------
%%% @doc Hard factory reset (sda1) + remove nl dir in sda -> Download (Upgrade NL) -> integrate.
%%% @spec c_1_6(Config) -> ok
%%% @end
%%%--------------------------------------------------------------------
c_1_6(Config) ->
    ct:pal("C_1_6: AI with sda1 + remove nl dir in sda2, Upgrade NL."),
    
    %%%%
    %% Precondition. Hard factory reset, to make nl on sda1 active + remove nl dir in sda2.
    %%%%
    ct:pal("Precondition: perform hard_factory_reset + remove nl dir in sda2."),
