%%% #0.    BASIC INFORMATION
%%% ----------------------------------------------------------
%%% %CCaseFile:	lmt_integration_on_site_conf_self_upgrade_utf8_bom_SUITE.erl %
%%% @author etxmlar
%%% @copyright Ericsson AB 2017
%%% @version /main/R8A/R9A/4
%%%
...
-module(lmt_integration_on_site_conf_self_upgrade_utf8_bom_SUITE).
-author('etxmlar').
-vsn('/main/R8A/R9A/4').
-date('2017-06-21').
...
%%% Rev        Date       Name        What
%%% -----      ---------  --------    ------------------------
%%% R8A/1      2017-01-18 etxmlar     Created 
%%% R8A/2      2017-01-18 etxmlar     Updated 
%%% R8A/3      2017-01-19 etxmlar     Updated 
%%% R8A/4      2017-01-23 etxmlar     Updated for DUS33/DUS53/c608
...
%%% @spec nodeUC640_N_012(Config) -> ok
%%% @end
%%%--------------------------------------------------------------------
nodeUC640_N_012(Config) ->
    ...
    TftpBootDir = proplists:get_value(tftpboot_dir, Config),
    add_bom(TftpBootDir++"config_initial.netconf"),
...
%%--------------------------------------------------------------------
add_bom(File) ->
    ct:pal("Add bom. Insert syntax fault in file: ~p ~n",  [File]),
    ct:pal("Our name on SiteBasic.xml is config_initial.netconf."),
    case file:read_file(File) of
	{ok, Bin} ->
	    BOM_bin = <<239,187,191>>,
	    With_bom = <<BOM_bin/binary, Bin/binary>>,
	    file:write_file(File, With_bom);
	{error, _} ->
	    ct:fail("TC will fail due File ~p need to exist to add a BOM~n", [File])
    end.
