%%% #0.    BASIC INFORMATION
%%% ----------------------------------------------------------
%%% %CCaseFile:	lmt_integration_on_site_conf_self_upgrade_utf8_faulty_bom_SUITE.erl %
%%% @author etxmlar
...
-module(lmt_integration_on_site_conf_self_upgrade_utf8_faulty_bom_SUITE).
-author('etxmlar').
-vsn('/main/R9A/2').
-date('2017-02-09').
...
%%% Rev        Date       Name        What
%%% -----      ---------  --------    ------------------------
%%% R9A/1      2017-02-09 etxmlar     Created 
%%% R9A/2      2017-02-09 etxmlar     Updated 
%%% ----------------------------------------------------------
%%%
...
end_per_testcase(TestCase, Config) 
  when TestCase == nodeUC640_N_012 ->
    ...
    %% Move back org_config_initial.netconf to config_initial.netconf
    ct:log("Move back org_config_initial.netconf to config_initial.netconf"),
    TftpBootDir = proplists:get_value(tftpboot_dir, Config),
...
%%%--------------------------------------------------------------------
%%% @doc Board restore (sda2) -> Download with a self Upgrade and a faulty SiteBasic.xml -> integrate.
%%% @spec nodeUC640_N_013(Config) -> ok
%%% @end
%%%--------------------------------------------------------------------
nodeUC640_N_013(Config) ->
   
    ct:pal("nodeUC640_N_012:~n" 
	   "Run on both Sec/UnSec board.~n"
	   "Board restore -> Download with a self Upgrade and a faulty SiteBasic.xml -> Integrate.", []),
    ...
    %% Add utf8 bom fault into config_initial.netconf.
    TftpBootDir = proplists:get_value(tftpboot_dir, Config),
    add_not_handled_bom(TftpBootDir++"config_initial.netconf"),
...
%%--------------------------------------------------------------------
%% Will make cleaning fail and interupt the AI. To test that NL clean DB and dont die

add_not_handled_bom(File)->
    ct:pal("Add not handle BOM. Insert syntax fault in file: ~p ~n",  [File]),
    ct:pal("Our name on SiteBasic.xml is config_initial.netconf."),
    case file:read_file(File) of
        {ok, Bin} ->
            Xml_start_bin = <<60>>,
            BOM_bin = <<239,187,191>>,
            With_bom = <<BOM_bin/binary,  
                         Xml_start_bin/binary,  %% Corr will erase until this
                         BOM_bin/binary, 
                         Bin/binary>>,
             file:write_file(File, With_bom);
         {error, _} ->
	    ct:fail("TC will fail due File ~p need to exist to add not handled BOM~n", [File])
    end.
...
cp_config_initial_to_org(Config) ->
    ...
    TftpBootDir = proplists:get_value(tftpboot_dir, Config),
    ct:pal("copy config_initial.netconf to org_config_initial.netconf."),
