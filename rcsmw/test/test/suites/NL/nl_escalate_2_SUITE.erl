%%% #0.    BASIC INFORMATION
%%% ----------------------------------------------------------
%%% %CCaseFile:	nl_escalate_2_SUITE.erl %
%%% @author etxmlar
%%% @copyright Ericsson AB 2015-2017
%%% @version /main/R4A/R7A/R9A/1
%%%
...
-module(nl_escalate_2_SUITE).
-author('etxmlar').
-vsn('/main/R4A/R7A/R9A/1').
-date('2017-04-20').
...
%%% Rev        Date       Name        What
%%% -----      ---------  --------    ------------------------.
%%% R4A/1      2015-09-28 etxmlar     Created. 
%%% R4A/1      2015-09-28 etxmlar     Updated. 
%%% R7A/1      2016-10-27 etxmlar     Update if nl on tcu is to old.  
%%% R7A/2      2017-01-11 etxmlar     Update the check, if nl is to old     
%%% R9A/1      2017-04-20 etxmlar     R7 NL can install X3 UPs from R9A
...
-export([suite/0,
	 ...
	 end_per_testcase/2,
	 all/0,
	 board_restore_hard_sda1_check/1,
	 export_ai_log/1,
...
%% @hidden
init_per_testcase(board_restore_hard_sda1_check = TestCase, Config) ->
...
groups() ->
    [
     {d_1_3, [], [board_restore_hard_sda1_check,
		  export_ai_log,
		  download_after_hfr,
		  integrate
		 ]}
...
%%--------------------------------------------------------------------
%% @doc
%% @spec board_restore_hard_sda1_check(Config) -> ok
%% @end
%%--------------------------------------------------------------------
board_restore_hard_sda1_check(Config) ->
    ct:pal("D_1_3: Board restore hard (hard_factory_reset)."
	   "Then check sda1 NL -> download -> integrate."),
