 ...
 * Rev        Date       Name        What
 * -----      -------    --------    --------------------------
 * R2A/1      2013-08-29 erarafo     First version
 * R2A/2      2013-08-29 erarafo     IWD candidate
 * R2A/3      2013-08-30 erarafo     IWD version
 * R2A/4      2013-09-02 erarafo     IWD PA3 version
 * R2A/5      2013-10-10 erarafo     Removed CPP legacy
 * R2A/6      2013-10-10 erarafo     Fixed compiler warning
 * R2A/7      2013-10-28 erarafo     Uplift to ICTI IWD PB1
 * ----------------------------------------------------------------------
 */
...
int main(void)
{
  ...
  if (r2 != SA_AIS_OK) {
    ...
    fprintf(log, "old schema version: %u, release: %u, correction: %u\n",
        getVersion(&old), getRelease(&old), getCorrection(&old));
    fprintf(log, "new schema version: %u, release: %u, correction: %u\n",
        getVersion(&new), getRelease(&new), getCorrection(&new));
  }

  if (getVersion(&old) == getVersion(&new) &&
      getRelease(&old) == getRelease(&new) &&
      getCorrection(&old) == getCorrection(&new)) {
