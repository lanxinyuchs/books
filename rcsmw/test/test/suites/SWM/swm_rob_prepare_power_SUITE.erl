%%% #0.    BASIC INFORMATION
%%% ----------------------------------------------------------
%%% %CCaseFile:	swm_rob_prepare_power_SUITE.erl %
%%% @author etxivri
%%% @copyright Ericsson AB 2014-2016
%%% @version /main/R2A/R3A/R7A/1
%%% 
%%% @doc == unexpected power off / on in phase of prepare.  ==
%%% <br/><br/>
%%% @end

-module(swm_rob_prepare_power_SUITE).
...
%%% R2A/5      2014-06-26 etxivri     Added wait for ntp synch
%%% R2A/6      2014-08-11 etxivri     Update due to new behaviour.
...
-export([build_to_up/1,
	 random_power/1,
	 procentage_10_power/1,
	 procentage_20_power/1,
	 procentage_30_power/1,
	 procentage_40_power/1,
	 procentage_50_power/1,
	 procentage_60_power/1,
	 procentage_70_power/1,
	 procentage_80_power/1,
	 procentage_90_power/1
	 ]).
...
random_power(_Config) ->
    %%%%
    %% Get a random nr between 1-90, that will be used in sleep before power.
    %%%%
     Procentage= rand:uniform(90),
    ct:pal("TC will use Procentage : ~p",[Procentage]),
    prepare_fail_due_to_power(Procentage).

procentage_10_power(_Config) ->
    Procentage = 10,
    ct:pal("TC will use Procentage : ~p",[Procentage]),
    prepare_fail_due_to_power(Procentage).
procentage_20_power(_Config) ->
    Procentage = 20,
    ct:pal("TC will use Procentage : ~p",[Procentage]),
    prepare_fail_due_to_power( Procentage).
procentage_30_power(_Config) ->
    Procentage = 30,
    ct:pal("TC will use Procentage : ~p",[Procentage]),
    prepare_fail_due_to_power(Procentage).
procentage_40_power(_Config) ->
    Procentage = 40,
    ct:pal("TC will use Procentage : ~p",[Procentage]),
    prepare_fail_due_to_power(Procentage).
procentage_50_power(_Config) ->
    Procentage = 50,
    ct:pal("TC will use Procentage : ~p",[Procentage]),
    prepare_fail_due_to_power(Procentage).
procentage_60_power(_Config) ->
    Procentage = 60,
    ct:pal("TC will use Procentage : ~p",[Procentage]),
    prepare_fail_due_to_power(Procentage).
procentage_70_power(_Config) ->
    Procentage = 70,
    ct:pal("TC will use Procentage : ~p",[Procentage]),
    prepare_fail_due_to_power(Procentage).
procentage_80_power(_Config) ->
    Procentage = 80,
    ct:pal("TC will use Procentage : ~p",[Procentage]),
    prepare_fail_due_to_power(Procentage).
procentage_90_power(_Config) ->
    Procentage = 90,
    ct:pal("TC will use Procentage : ~p",[Procentage]),
    prepare_fail_due_to_power(Procentage).
...
%%% @end
%%%--------------------------------------------------------------------
prepare_fail_due_to_power(Value) ->
    ct:pal("UG. unexpected power in prepare phase.",[]),
    ...
    ct:pal("Wait for prog procentage : ~p, then power node.",[Value]),
