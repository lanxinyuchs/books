...
%%% @version /main/R1A/R2A/R3A/R4A/R5A/R7A/R8A/R9A/R10A/R11A/5
-module(basic_tests_SUITE).
-id('Updated by CCase').
-vsn('/main/R1A/R2A/R3A/R4A/R5A/R7A/R8A/R9A/R10A/R11A/5').
-date('2017-11-13').
-author('erarube').
%%% ----------------------------------------------------------
...
%%% R9A/3      2017-06-21 erarube     Increase of CpuMemory check from 1000 to 1050 for RAT due to dus53 on the limit.
%%% R10A/1     2017-06-21 erarube     Allow SNMP trap {type,nsNotifyShutdown} since at power cycle of C608 is seen often
%%%                                   which is sent when COM is restarted, and that can be triggered by the system in this situation.
%%% R11A/2     2017-09-05 etxivri     revert to R11A/0
%%% R11A/4     2017-10-10 etxkols     revert to R11A/2
%%% R11A/5     2017-11-13 erarube     Support for dus5201tk and dus3201tk
%%% ----------------------------------------------------------
%%% 
%%% #2.    EXPORT LISTS
%%% ----------------------------------------------------------
...
wait_for_login_prompt() ->
	...
	Other ->
		...
		BOARDTYPE when BOARDTYPE == "tcu03";
			       ...
			       BOARDTYPE == "dus5201tk"; %dus52tk (dus52 from R9)
			       BOARDTYPE == "dus3201tk"; %dus32tk (dus32 from R9)
			       BOARDTYPE == "dus5201";
			       BOARDTYPE == "dus3201";
			       BOARDTYPE == "c608";
