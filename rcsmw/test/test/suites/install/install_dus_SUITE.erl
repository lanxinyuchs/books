...
%%% @version /main/R1A/R2A/R3A/R4A/R6A/R7A/R8A/R9A/R10A/R11A/1
%%% @doc ==Installs du board and checks software version==
%%% @end

-module(install_dus_SUITE).
-id('Updated by CCase').
-vsn('/main/R1A/R2A/R3A/R4A/R6A/R7A/R8A/R9A/R10A/R11A/1').
-date('2017-11-10').
-author('erarube').
...
%%% R11A/1     2017-11-10 erarube     Support for dus3201tk/dus5201tk
%%% ----------------------------------------------------------
%%% #2.    EXPORT LISTS
%%% ----------------------------------------------------------
...
install_networkloader(N) ->
    ...
    Cmd = case BOARDTYPE of
		  ...
		  "dhcp;setenv bootargs console=ttyAMA0 earlyprintk debug initrd=0x8000000,32M rdinit=/addons/bin/labloader.sh;rcs 1;mw.b 8000000 0 2000000; tftp 4000000 " ++ Hw ++ "/linux.img;tftp 8000000 " ++ Hw ++ "/bootfs.cpio.gz;imxtract 4000000 cpm2_52_fdt 5200000;merge_dtb 5200000 5000000;bootm 4000000 - 5000000";
	      "dus3201tk" -> %dus32 tk (dus32 from R9 and up)
		  "dhcp;setenv bootargs console=ttyAMA0 earlyprintk debug initrd=0x8000000,32M rdinit=/addons/bin/labloader.sh;rcs 1;mw.b 8000000 0 2000000; tftp 4000000 " ++ Hw ++ "/linux.img;tftp 8000000 " ++ Hw ++ "/bootfs.cpio.gz;imxtract 4000000 cpm2_32_fdt 5200000;merge_dtb 5200000 5000000;bootm 4000000 - 5000000";
...
break_uboot_arm(Console,Power, BOARDTYPE,N,M) ->
	...
	ok ->
	    Uboot = case BOARDTYPE of
			...
			BOARDTYPE when BOARDTYPE == "dus3201tk" ->
			    case ct_telnet:expect(Console, ["Ericsson Version: 7/CXC1740623/32.*\\)"],[{timeout,30000}]) of
				{ok, Match} ->
				    case re:run(Match,"Ericsson Version: [0-9]/CXC17.*/32_(.*) \\(",[{capture,[1],list}]) of
					{match,[Rev]} ->
					    ct:pal("Uboot: ~s",[Rev]),
					    other
				    end;
			...
			BOARDTYPE when BOARDTYPE == "dus5201tk" ->
			    case ct_telnet:expect(Console, ["Ericsson Version: 7/CXC1740623/52.*\\)"],[{timeout,30000}]) of
				{ok, Match} ->
				    case re:run(Match,"Ericsson Version: [0-9]/CXC17.*/52_(.*) \\(",[{capture,[1],list}]) of
					{match,[Rev]} ->
					    ct:pal("Uboot: ~s",[Rev]),
					    other
				    end;
