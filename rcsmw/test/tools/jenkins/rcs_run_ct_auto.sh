...
prep()
{
 PREP_SCRIPT=$(cat ${WORKSPACE}/info.txt  | grep PREP_SCRIPT | cut -f2- -d= )
 if [ "${PREP_SCRIPT}" != "" ]; then
   PREP_PROJECT=$(cat ${WORKSPACE}/info.txt  | grep PREP_PROJECT | cut -f2- -d= )
   PREP_LAB=$(cat ${WORKSPACE}/info.txt  | grep PREP_LAB | cut -f2- -d= )
   PREP_FLAVOR=$(cat ${WORKSPACE}/info.txt  | grep PREP_FLAVOR | cut -f2- -d= )
   PREP_NETWORK=$(cat ${WORKSPACE}/info.txt  | grep PREP_NETWORK | cut -f2- -d= )
   X="PREP_SCRIPT=$PREP_SCRIPT PREP_PROJECT=$PREP_PROJECT PREP_LAB=$PREP_LAB PREP_FLAVOR=$PREP_FLAVOR PREP_NETWORK=$PREP_NETWORK" 
   echo $X
   PREP_CONFIG=""
   if [ "${PREP_PROJECT}" != "" ]; then
     PREP_CONFIG="$PREP_CONFIG -p ${PREP_PROJECT}"
   fi
   if [ "${PREP_LAB}" != "" ]; then
     PREP_CONFIG="$PREP_CONFIG -w ${PREP_LAB}"
   fi
   if [ "${PREP_FLAVOR}" != "" ]; then
     PREP_CONFIG="$PREP_CONFIG -f ${PREP_FLAVOR}"
   fi
   if [ "${PREP_NETWORK}" != "" ]; then
     for i in $(echo $PREP_NETWORK | sed 's/:/ /g' )
       do
         PREP_CONFIG="$PREP_CONFIG -n ${i}"
       done
   fi

   ${PREP_SCRIPT} -l $NODE -c $PREP_UP $PREP_CONFIG
else
  $RCT_PREP $NODE $PREP_UP $PREP_OAMP
fi
...
if [ -f ${WORKSPACE}/sdc_sim.cfg ]; then 
   cat ${WORKSPACE}/sdc_sim.cfg | grep -v "%%" >> $PWD/auto_config.cfg
fi 
if [  -f ${WORKSPACE}/properties_$(basename $SPEC) ]; then 
   if [  -f ${WORKSPACE}/info.txt ]; then 
     LL=$(cat ${WORKSPACE}/info.txt )
     PRE_BDC_CONFIG=$(cat ${WORKSPACE}/properties_$(basename $SPEC)  | grep PRE_BDC_CONFIG | cut -f2- -d= )
   ...
   if [ "$(echo $PRE_BDC_CONFIG | cut -c2-4 )" = "rat" ]; then
      U=rat
   elif [ "$PRE_BDC_CONFIG" != "" ]; then
      U=$PRE_BDC_CONFIG
   else
      U=dummy
   fi
...
echo "$RCT_RUN -sim $NODE ${WRAPPER} ${CLUSTER} -spec $SPEC -config $AUTO_CONFIG_FILE -create_priv_dir auto_per_tc -logdir $NEWLOGDIR   -noshell -ct_hooks cth_surefire "[{path,\"${PWD}/report_3.${REPEAT}.xml\"}]"  -pa ${NEWLOGDIR}/dev_patches/  " 
    $RCT_RUN -sim $NODE ${WRAPPER} ${CLUSTER} -spec $SPEC -config $AUTO_CONFIG_FILE -create_priv_dir auto_per_tc -logdir $NEWLOGDIR   -noshell -ct_hooks cth_surefire "[{path,\"${PWD}/report_3.${REPEAT}.xml\"}]"  -pa ${NEWLOGDIR}/dev_patches/  
    else
