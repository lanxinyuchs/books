...
*/
	...
	//Here first read srcPort , and based on this value , find correspond bufferindex
        ECP_RX_MEM_RD(edev->regs, ch, ecp_chn_offset, reg);
	size_pad -= 4;
        srcPort = (ntohl(reg) >> 24) & 0xFF;
	if (srcPort > ECP_CIDS)
	{
          log_err("ABN:invalid buffer index: 0x%x", srcPort");
	   goto out_err;
	}
        econ = edev->conns[srcPort];

	if (unlikely(!econ)) {
		log_err("econ is NULL");
		++edev->stats.data[srcPort].rx_addr_errors;
		goto out_err;
	}
	...
	pkt = ulh_tbuff_get(tbuff);
        ...
        Tempdata = (ntohl(reg) << 8) & 0xFFFFFF00;
	...
	*cid = econ->cid;
	while (size_pad) {
		...
		*(uint32_t *)pkt = Tempdata | ((ntohl(reg)>> 24) & 0xFF);
		Tempdata  = (ntohl(reg) << 8)& 0xFFFFFF00;
