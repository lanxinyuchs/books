...
static struct rbs_sys_bt_deps dus53_bt = {
	.bt_ops = &rbs_sys_cpm_bt_ops,
	.bt_fn_depend = rbs_sys_fn_deps,
};

static int __init dus53_bt_init(void)
{
	rbs_sys_add_bt_ops(&dus53_bt, RBS_BT_DUS53);
	return 0;
}
postcore_initcall_sync(dus53_bt_init);
