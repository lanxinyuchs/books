...
struct child * child_create(char **argv, char **env, struct domain *d,
...
void child_launch(struct child *ch, struct state *s)
{
	...
	if (child_pid == -1) {
		...
		if (s->global_env) {
			for (int i = 0; s->global_env[i]; i += 2) {
				if (setenv(s->global_env[i], s->global_env[i + 1], 1))
					CB(reboot, "setenv failed");
			}
		}

		/* Set child specific environment */
		if (ch->env) {
			for (int i = 0; ch->env[i]; i += 2) {
				if (setenv(ch->env[i], ch->env[i + 1], 1))
					CB(reboot, "setenv failed");
			}
		}
...
void child_died(struct child *ch, struct state *s)
{
	...
	else if (ch->terminated_sockfd >= 0) {
		child_set_state(ch, CHILD_STATE_STOPPED);
