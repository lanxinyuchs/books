...
#define child_dump(list, s) do {                   \
	struct child *_ch;                             \
	int idx = 1;                                   \
	log_info("Dumping child list: " #list "\n");   \
	child_foreach(_ch, list, s)                    \
		log_info("%d: %s\n", idx++, _ch->argv[0]); \
} while (0)

#define child_in_list(ch, list, s) ({       \
	struct child *_ch;                      \
	int _res = 0;                           \
	child_foreach(_ch, list, s) {           \
		if (_ch == ch)                      \
...
#define child_from_id(f_id, s) ({           \
	struct child *_ch;                      \
	struct child *_ch_res = NULL;           \
	child_foreach(_ch, cglist, s) {         \
		if (_ch->id == f_id) {              \
			_ch_res = _ch;                  \
			break;                          \
		}                                   \
...
#define child_count(list, s) ({             \
	struct child *_ch;                      \
	int _cnt = 0;                           \
	child_foreach(_ch, list, s) _cnt++;     \
	_cnt;                                   \
})

#define child_delete_all(list, s) do {        \
	struct child *_next;                      \
	struct child *_ch = child_first(list, s); \
	while (_ch) {                             \
		_next = child_next(_ch, list);        \
		child_delete(_ch);                    \
		_ch = _next;                          \
	}                                         \
} while (0)
...
extern struct child * child_create(char **argv, char **env, struct domain *d,
                                   uint64_t id, struct child_config *cfg);
extern void child_delete(struct child *ch);
extern void child_launch(struct child *ch, struct state *s);
