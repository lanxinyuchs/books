...
/** ==================================================================== */
/**
 *   Extracts command line tokens and then executes the command
 *
 *   @param tok        Readline tokenizer
...
static int exec_command_line(Tokenizer *tok, char *line)
{
	int    local_argc, ret = 0;
	char **local_argv;

	if (tok_str(tok, line,
	            &local_argc, (const char***) &local_argv) == 0 ) {
...
/** ==================================================================== */
/**
 *   Reads command line from stdin. This replaces readline which echoes.
 *
 *   @param promt      The prompt
 *
 *   @return           NULL, or the command line string.
 *
 *   @par Globals:
 *                     --
 */
...
static char* echoless_readline(char *prompt)
{
	char *buf, *line;
	...
	line = fgets(buf, LINE_SIZE, stdin);
	if (line == NULL) {
		fprintf(stderr,
		        "ERROR: fgets(stdin) failed, errno: %d\n", errno);
		free(buf);
	}
...
{
	int opt, ret, echoless = 0;
	char *line, *s, *prompt, *command = NULL;
	struct termios attr;
	Tokenizer *tok;
	...
	while ((opt = getopt(argc, argv, "pc:")) != -1) {
		...
		default:
			fprintf(stderr,
			        "Usage: %s [-p] [-c '<command line>']\n"
			        "-h   This help message\n"
			        "-p   Disable echo\n"
			        "-c   Execute a command without a prompt\n",
			        argv[0]);
			exit(EXIT_FAILURE);
		}
	}
	...
	if (command != NULL) {
		ret = exec_command_line(tok, command);
		if (echoless) {
			attr.c_lflag |= ECHO;
			tcsetattr(STDIN_FILENO, TCSANOW, &attr);
		}
