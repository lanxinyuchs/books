 ...
 */
/* ===================================================================== */
static void print_short_help(char *name, char *help)
{
	printf("%-16s%s\n", name, help);
}
 ...
 */
/* ===================================================================== */
static int list_commands(char *expr)
{
	char  *prev;
	char  *next_local;
	char  *next_remote;
	...
	*prev = '\0';
	...
	while (1) {
		...
		next_linux = whitelist_get_next(expr, prev);

		if (!next_local && !next_remote)      next = next_linux;
		else if (!next_local && !next_linux)  next = next_remote;
		else if (!next_remote && !next_linux) next = next_local;
		else if (!next_linux)
			next = strcmp(next_local, next_remote) < 0 ? next_local : next_remote;
		else if (!next_local)
			next = strcmp(next_remote, next_linux) < 0 ? next_remote : next_linux;
		else if (!next_remote)
			next = strcmp(next_local, next_linux) < 0 ? next_local : next_linux;
		else {
			char *candidate = strcmp(next_local, next_remote) < 0?
			                  next_local : next_remote;
			next = strcmp(candidate, next_linux) < 0 ? candidate : next_linux;
		}

		if (next) {
			if (next == next_local)
				help = local_command_help(next);
			else if (next == next_remote)
				help = remote_command_help(next);
			else
				help = whitelist_get_help(next);
 ...
 */
/* ===================================================================== */
static void print_help(char *name, char *usage, char *description)
{
	...
	if ( ptr ) {
		...
		if ( (str = calloc(1, strlen(ptr))) != NULL ) {
			strcpy(str, ptr);
			ptr = strtok_r(str, "\n", &saveptr);
 ...
 */
/* ===================================================================== */
static int help_command(char *name)
{
	char  *usage;
	char  *descr;

	if ( local_command(name) ) {
		descr = local_command_descr(name);
		usage = local_command_descr(name);
		if (usage && descr)
			print_help(name, usage, descr);
		return 0;
	}

	usage = whitelist_get_usage(name);
	if (usage) {
		descr = whitelist_get_descr(name);
		if (descr)
			print_help(name, usage, descr);
		else
