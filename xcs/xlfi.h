...
/** xlf_separate_header::magic value for an XLF header. */
#define XLF_SEPARATE_HDR_MAGIC              0x58504C48

/**
 * xlf_separate_header major version value.
 * Stepped when the interface is NOT backwards compatible.
 * In case it is stepped, @ref XLF_SEPARATE_HDR_MINOR_VERSION should be reset.
 */
#define XLF_SEPARATE_HDR_MAJOR_VERSION      1

/**
 * xlf_separate_header minor version value.
 * Stepped when the interface is backwards compatible but have been extended.
 * Reset when @ref XLF_SEPARATE_HDR_MAJOR_VERSION is stepped.
 * 0: First version.
 */
#define XLF_SEPARATE_HDR_MINOR_VERSION      0
...
/**
 * xlf_blob_header::type value for a BLOB LM that contains Firmware, e.g. FPGA
 * or SERDES.
 * xlf_blob_header::name is mandatory and defines which component the LM applies
 * for.
 */
#define XLF_BLOB_TYPE_FW        6

/**
 * xlf_blob_header::type value for a BLOB LM that contains u-boot script.
 * xlf_blob_header::name is mandatory to define.
 */
#define XLF_BLOB_TYPE_BOOT_SCRIPT  7
...
/**
 * @brief XLF header format for separate header.
 *
 * This header precedes an XLF packaged in a signed container used for
 * secure boot.
 * The contents of the signed container is CRC checked which prevents
 * xlf_iboot_header::update and xlf_iboot_header::seq_number fields to be
 * updated in runtime. Instead the xlf_separate_header::update
 * xlf_separate_header::seq_number fields in this separated header is used.
 * The size of the header shall be 256 bytes.
 * All multi byte numeric fields shall be in Big Endian byte order.
 */
struct xlf_separate_header {
	/**
	 * Magic value equal to @ref XLF_SEPARATE_HDR_MAGIC.
	 */
	uint32_t magic;
	/**
	 * Major version.
	 * 1 = this version, see @ref XLF_SEPARATE_HDR_MAJOR_VERSION.
	 * Stepped when the interface is NOT backwards compatible.
	 * If SW encounters a major version which differs from its
	 * @ref XLF_SEPARATE_HDR_MAJOR_VERSION then it should treat the
	 * signed XLF container that follows this header as invalid.
	 */
	uint32_t major_version;
	/**
	 * Minor version.
	 * 0 = this version, see @ref XLF_SEPARATE_HDR_MINOR_VERSION.
	 * Stepped when the interface is backwards compatible but have been
	 * extended.
	 * Reset when major_version is stepped.
	 * SW should treat the signed XLF container that follows this header as
	 * valid even if it encounters a minor version which differs from its
	 * @ref XLF_SEPARATE_HDR_MINOR_VERSION.
	 * If SW encounters a minor version which is lower than its
	 * @ref XLF_SEPARATE_HDR_MINOR_VERSION then it should only use the fields
	 * in this header that is applicable for the lower encountered version.
	 * SW that updates this header should use the fields that is applicable
	 * for its @ref XLF_SEPARATE_HDR_MINOR_VERSION.
	 */
	uint32_t minor_version;
	/**
	 * Update field.
	 * bit 0: Indicates working status, see @ref XLF_IBOOT_HDR_UPDATE_WORKING_BIT.
	 *        Present from version 1.0
	 */
	uint32_t update;
	/**
	 * Sequence number. Present from version 1.0
	 */
	uint32_t seq_number;
	/**
	 * Reserved bytes to be used when interface is extended.
	 * Shall be 0xffffffff.
	 */
